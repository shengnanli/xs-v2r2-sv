// =============================================================================
// xs_EntriesVfmaVialuFixVimacVppuVfaluVfcvtVipuVsetrvfwvf_core
//   —— 香山 V2R2(昆明湖) 发射队列条目阵列 可读重写
//      (VfmaVialuFixVimacVppuVfaluVfcvtVipuVsetrvfwvf 变体, hasCompAndSimp 路)
// -----------------------------------------------------------------------------
// 设计源:src/main/scala/xiangshan/backend/issue/Entries.scala
// golden 对照:EntriesVfmaVialuFixVimacVppuVfaluVfcvtVipuVsetrvfwvf.sv
//   (例化 2×EnqEntry_14 + 2×OthersEntry_136(simp) + 12×OthersEntry_138(comp)
//    + 2×EnqPolicy_14(simp/comp 转移策略))
//
// ★ 本模块 = 条目阵列 + 转移策略 + 双端口年龄最老选择(oldest mux)★
// 单条目「唤醒-选择」逻辑全在 xs_iq_entry_vfmavialufixvimacvppuvfaluvfcvtvipuvsetrvfwvf;
// 本层做:
//   1) 例化 16 个单条目:2 enq(IS_ENQ) + 2 simp(IS_TRANS) + 12 comp。
//   2) 转移策略(hasCompAndSimp 分支):simp 为转移源,提级到 comp。
//   3) ★ 双 deq 端口年龄最老选择(numDeq=2):每个端口独立 comp>simp>enq 三级 Mux ★。
//   4) 阵列状态汇聚:valid/issued/canIssue/fuType/dataSources。
//
// ★ 与样板 EntriesVfmaVialuFixVfalu 的差异 ★:仅条目内部字段(8 位 fuType + 透传
//   rf_wen/vl_wen/sel_imm/imm)不同——本层骨架(转移策略/双口年龄选择/Mux1H 汇聚)逐字
//   同构,新增字段随 entry_t 自动透传(oldest mux / 转移 Mux1H 整条目 OR 累加,无需改动)。
//
// X 铁律:oldest / Mux1H 用「sel[i] ? entry[i] : 0」OR 累加,sel=0 不引 X。
// =============================================================================
module xs_EntriesVfmaVialuFixVimacVppuVfaluVfcvtVipuVsetrvfwvf_core
  import iq_vfmavialufixvimacvppuvfaluvfcvtvipuvsetrvfwvf_pkg::*; (
  input  logic clock,
  input  logic reset,
  ...
  // ===========================================================================
  // 3. 转移策略(hasCompAndSimp 分支,comp 数 12)
  // ===========================================================================
  ...
  // finalCompTransSelVec(enq):两支都用 selValid gate 的 selOH。
  ...
  generate
    for (e = 0; e < NUM_ENTRIES; e++) begin : g_entry
      ...
      xs_iq_entry_vfmavialufixvimacvppuvfaluvfcvtvipuvsetrvfwvf
        #(.IS_ENQ(IS_ENQ_E), .IS_TRANS(IS_TRANS_E)) u_ety (
        .clock              (clock),
        .reset              (reset),
