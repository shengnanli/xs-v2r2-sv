...
// =============================================================================
// SplittedSRAMTemplate_23 —— 逻辑 512set×8way×2bit；**setSplit=2**（新形态）
//   → 2 个 SRAMTemplate_64(256set×8way×2bit, bp_tage_bt 双口)：
//        偶 set(setIdx[0]=0) → array_0_0_0    奇 set → array_1_0_0
//   内层地址 = setIdx[8:1]（去掉 LSB）。TageBTable 的存储。
// 关键：SRAM 读出有 1 拍延迟，故把"本拍读选了哪个 bank"寄存成 ren_vec，
//      下一拍用 Mux1H 选出对应 bank 的读响应。写按 bank 用 valid 门控。
// 内层 SRAMTemplate_64 为已验证子模块（FM 当黑盒）。
// =============================================================================
