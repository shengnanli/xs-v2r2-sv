// 自动生成:scripts/gen_iq_ffmacfdiv.py —— 勿手改
// EntriesFaluFmacFdiv golden 同名扁平 wrapper —— 例化可读核 xs_EntriesFaluFmacFdiv_core。
module EntriesFaluFmacFdiv import iq_ffmacfdiv_pkg::*; (
  input  clock,
  input  reset,
  ...
  always_comb begin
    ...
    // 外部只送 fuType_{11,12,14} 三位,打包进紧凑存储(见 pack_fu_type)
    ...
    c_og1resp1_resp = io_og1Resp_1_bits_resp;
  ...
  assign io_deqEntry_0_bits_payload_fpWen = c_deq_entry[0].payload.fp_wen;
  assign io_deqEntry_0_bits_payload_fpu_wflags = c_deq_entry[0].payload.fpu_wflags;
  assign io_deqEntry_0_bits_payload_fpu_fmt = c_deq_entry[0].payload.fpu_fmt;
  assign io_deqEntry_0_bits_payload_fpu_rm = c_deq_entry[0].payload.fpu_rm;
  ...
  assign io_deqEntry_1_bits_payload_fpu_fmt = c_deq_entry[1].payload.fpu_fmt;
  ...
  xs_EntriesFaluFmacFdiv_core u_core (
