...
// ============================================================================
// xs_ExuBlock_1_core —— 执行单元簇容器(香山 V2R2 昆明湖)
// ----------------------------------------------------------------------------
// 设计意图来源:src/main/scala/xiangshan/backend/exu/ExuBlock.scala  class ExuBlock
//
// ExuBlock_1 把一个调度块(SchdBlock)里的全部 ExeUnit(执行单元)拼成一簇:
...
// ============================================================================
module xs_ExuBlock_1_core
  import exublock_1_pkg::*;
(
  input  clock,
  input  reset,
  `include "exublock_1_ports.svh"
);

  `include "exublock_1_decls.svh"    // 子模块输出网 + 流水寄存器状态声明
  `include "exublock_1_logic.svh"    // §1 frm 流水 / §2 error 流水(可读重写)
  `include "exublock_1_connect.svh"  // ExeUnit 黑盒例化 + 端口机械连线
