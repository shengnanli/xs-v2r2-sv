// =============================================================================
// xs_EntriesVfmaVialuFixVfalu_core —— 香山 V2R2(昆明湖) 发射队列条目阵列 可读重写
//                                    (VfmaVialuFixVfalu 变体, hasCompAndSimp 路)
// -----------------------------------------------------------------------------
// 设计源:src/main/scala/xiangshan/backend/issue/Entries.scala
// golden 对照:EntriesVfmaVialuFixVfalu.sv(例化 2×EnqEntry_16 + 2×OthersEntry_150(simp)
//             + 12×OthersEntry_152(comp) + 2×EnqPolicy_14(simp/comp 转移策略))
//
// ★ 本模块 = 条目阵列 + 转移策略 + 双端口年龄最老选择(oldest mux)★
// 单条目「唤醒-选择」逻辑全在 xs_iq_entry_vfmavialufixvfalu;本层做:
//   1) 例化 16 个单条目:2 enq(IS_ENQ) + 2 simp(IS_TRANS) + 12 comp。
//   2) 转移策略(hasCompAndSimp 分支):simp 为转移源,提级到 comp(与 VfdivVidiv 同构,仅 comp 数 6→12)。
//   3) ★ 双 deq 端口年龄最老选择(numDeq=2):每个端口独立 comp>simp>enq 三级 Mux ★。
//   4) 阵列状态汇聚:valid/issued/canIssue/fuType/dataSources。
//
// ★ 与 VfdivVidiv 样板(EntriesVfdivVidiv)的差异(全部增量)★
//   * numEntries 10→16(2/2/12),deqSelOH/oldestSel 位宽随之变化。
//   * ★ numDeq 1→2 ★:
//       - deqSel[e]          = OR over deq 端口:(deqSelOH_p.valid & deqSelOH_p.bits[e])
//       - deqPortIdxWrite[e] = 端口1 是否选中该条目(端口0 选中=0)
//       - issueResp 按条目 deqPortIdxRead 路由:idx==1 用 og*Resp_1,否则 og*Resp_0;
//         其中 og2 端口0 用真 resp(og2resp0_resp),端口1 用常量 SUCCESS(2'h3)。
//       - deqEntry 输出由单口扩为 2 口,各用自己的 oldestSel 做三级 Mux。
//   * 其余转移策略 / 年龄选择骨架与 VfdivVidiv 逐字同构。
//
// X 铁律:oldest / Mux1H 用「sel[i] ? entry[i] : 0」OR 累加,sel=0 不引 X。
// =============================================================================
module xs_EntriesVfmaVialuFixVfalu_core import iq_vfmavialufixvfalu_pkg::*; (
  input  logic clock,
  input  logic reset,
  ...
  // ---- 发射响应(每 deq 端口一组 og0/1/2)----
  input  og_resp_t [NUM_DEQ-1:0] og_resp,

  // ---- 发射选择(每 deq 端口一个 one-hot)----
  input  logic [NUM_DEQ-1:0]                 deq_sel_oh_valid,
  input  logic [NUM_DEQ-1:0][NUM_ENTRIES-1:0] deq_sel_oh_bits,

  // ---- 各端口年龄最老选择(每端口一组 enq/simp/comp oldestSel)----
  ...
  logic [NUM_ENTRIES-1:0]                      ety_deq_port_idx;
  ...
  // ===========================================================================
  // 1. ★ deqSel / deqPortIdxWrite(numDeq=2)★
  //    deqSel[e]          = 任一端口 one-hot 选中该条目
  //    deqPortIdxWrite[e] = 端口1 选中该条目(端口0 选中→写 0)
  // ===========================================================================
  always_comb begin
    for (int e = 0; e < NUM_ENTRIES; e++) begin
      logic sel;
      sel = 1'b0;
      for (int p = 0; p < NUM_DEQ; p++)
        sel |= deq_sel_oh_valid[p] & deq_sel_oh_bits[p][e];
      ety_deq_sel[e]            = sel;
      ety_deq_port_idx_write[e] = deq_sel_oh_valid[1] & deq_sel_oh_bits[1][e];
    end
  end

  // ===========================================================================
  // 2. ★ issueRespVec:每条目按 deqPortIdx 选端口,再按 issueTimer 选响应窗口 ★
  //    timer0→og0.valid/BLOCK; timer1→og1.valid/UNCERTAIN; timer2→og2.valid/真resp;
  //    timer3→0。端口0 的 og2 resp 用真值;端口1 的 og2 resp 恒 SUCCESS(2'h3)。
  // ===========================================================================
  always_comb begin
    for (int e = 0; e < NUM_ENTRIES; e++) begin
      logic [1:0] tmr;
      logic       idx;
      logic       og0v, og1v, og2v;
      logic [1:0] og2r;
      logic       v;
      resp_type_e r;
      tmr = ety_issue_timer[e];
      idx = ety_deq_port_idx[e];
      // 按本条目记录的 deq 端口选 og 响应组
      og0v = idx ? og_resp[1].og0_valid : og_resp[0].og0_valid;
      og1v = idx ? og_resp[1].og1_valid : og_resp[0].og1_valid;
      og2v = idx ? og_resp[1].og2_valid : og_resp[0].og2_valid;
      // 端口1 的 og2 resp 用常量 SUCCESS;端口0 用真 resp。
      og2r = idx ? 2'(RESP_SUCCESS) : og_resp[0].og2_resp;
      v = 1'b0; r = RESP_BLOCK;
      if (tmr == 2'h0) begin
        v = og0v; r = RESP_BLOCK;
      end else if (tmr == 2'h1) begin
        v = og1v; r = RESP_UNCERTAIN;
      end else if (tmr == 2'h2) begin
        v = og2v; r = resp_type_e'(og2r);
      end
  ...
  // ===========================================================================
  // 3. 转移策略(hasCompAndSimp 分支,与 VfdivVidiv 同构,comp 数 12)
  // ===========================================================================
  ...
  // finalCompTransSelVec(enq):两支都用 selValid gate 的 selOH(与 VfdivVidiv 同构)。
  ...
  generate
    for (e = 0; e < NUM_ENTRIES; e++) begin : g_entry
      ...
      xs_iq_entry_vfmavialufixvfalu #(.IS_ENQ(IS_ENQ_E), .IS_TRANS(IS_TRANS_E)) u_ety (
        .clock              (clock),
        .reset              (reset),
  ...
  // ===========================================================================
  // 7. ★ 双端口年龄最老选择(oldest mux):每端口独立做 comp>simp>enq 三级选择 ★
  //    deqEntry[p] = compSel[p].valid ? compOldest[p]
  //                : (simpSel[p].valid ? simpOldest[p] : enqOldest[p])
  // ===========================================================================
  always_comb begin
    for (int p = 0; p < NUM_DEQ; p++) begin
      entry_t enq_oldest, simp_oldest, comp_oldest;
      enq_oldest = '0; simp_oldest = '0; comp_oldest = '0;
      for (int i = 0; i < NUM_ENQ; i++)
        if (enq_oldest_sel_bits[p][i]) enq_oldest |= ety_entry[i];
      for (int i = 0; i < NUM_SIMP; i++)
        if (simp_oldest_sel_bits[p][i]) simp_oldest |= ety_entry[SIMP_BASE + i];
      for (int i = 0; i < NUM_COMP; i++)
        if (comp_oldest_sel_bits[p][i]) comp_oldest |= ety_entry[COMP_BASE + i];
      o_deq_entry[p] = comp_oldest_sel_valid[p] ? comp_oldest
                     : (simp_oldest_sel_valid[p] ? simp_oldest : enq_oldest);
    end
  end
