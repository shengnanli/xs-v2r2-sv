// =============================================================================
//  tlbuffer20_pkg —— TLBuffer_20 (TileLink A/D 两通道 skid 缓冲) 类型包
// -----------------------------------------------------------------------------
//  TLBuffer_20 是叶子级时序缓冲: A 请求 (client->manager)、D 应答 (manager->client)
//  各插一个 Queue2 (两拍 skid FIFO)。本层不解释 opcode/param 语义, 只切断 ready/valid
//  长组合路径并保持单通道 FIFO 顺序。把 golden 的扁平端口聚合成 A/D 两个 bundle, 便于
//  按 TL 通道方向阅读。
//  本变体特征: 窄总线缓冲: source 3 位、size 2 位、address 30 位、data 64 位, 无 user 字段。
//    A: client -> manager  请求   (enq=in_a, deq=out_a)
//    D: manager -> client  应答   (enq=out_d, deq=in_d)
// =============================================================================
package tlbuffer20_pkg;
