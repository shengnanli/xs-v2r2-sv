...
// cbo）→ srcType/lsrc 组装 → 黑盒 FPDecoder 取浮点控制位 → 异常合成。
// 覆盖逻辑的常量值（如 prefetch fuOpType=0xA/0x9/0x8）直接取自 golden 的最终
// io_deq 赋值表达式，保证与 golden 等价。
//
// 子模块 FPDecoder / UopInfoGen 作为 golden 黑盒例化。
// ============================================================================
`include "decodeunit_svh.svh"

module xs_DecodeUnit_core
...
(
  ...
  // --------------------------------------------------------------------------
  // 10. 黑盒 FPDecoder（浮点控制位 rm/wflags）
  // --------------------------------------------------------------------------
  ...
  // golden 把 FPDecoder 的 typeTagOut/typ/fmt 直接接到输出端口（见 golden 5560 行）


  // --------------------------------------------------------------------------
  // 11. 黑盒 UopInfoGen（向量/AMOCAS 拆分 uop 数）
  //     输入预解析信息全部由指令编码 + vtype 决定。
  // --------------------------------------------------------------------------
  // 与 golden UopInfoGen 例化一致：纯由指令编码 / raw fuOpType 决定
  ...
  always_comb begin
    ...
    // fpu_wflags = 黑盒 FPDecoder.wflags | 译码表 wfflags（golden 同式）
