...
```
cd verif/ut/Predictor && make fm   # 期望 SUCCEEDED，0 failing / 0 unmatched 比对点
```
...
> 关键设计决策：可读核采用“校验伴随”模式（独立可读再实现 + UT 探针证伪 + FM 等价），
> 而非侵入式替换 4 万行 body。这样既给出干净的学习载体（247 行可读 FSM/topdown），
> 又保证整模块 FM/UT 与 golden 逐字等价、零假阳性。与 [Composer](Composer.md) 的
> wrapper-照搬-黑盒策略一脉相承。
