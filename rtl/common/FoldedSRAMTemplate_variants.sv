...
//
// 对应 Chisel: utility.sram.FoldedSRAMTemplate —— 把一个深的逻辑 SRAM 在
// 物理上"折叠"成更宽、更浅的内层 SRAM：把 width 个连续 set 折叠到内层同一
// 物理 set 的 width 组 way 上。
//   - 内层物理 set = 逻辑 setIdx >> log2(width)（取高位）
//   - 折叠组 ridx  = 逻辑 setIdx 低 log2(width) 位（寄存一拍后选输出）
//   - 写 waymask  : 仅命中 ridx 选中那一组的 way 置位（width 组 × singlePort way）
//   - 读输出      : 用寄存的 ridx 在 width 组里做 one-hot 选择
// 内层为已验证的 SplittedSRAMTemplate 子模块（FM 时当黑盒）。
//
// 已覆盖变体：
//   FoldedSRAMTemplate   : 逻辑 2048set×2way×1b(bp_tage_us)，width=8 →
//       内层 SplittedSRAMTemplate_3 (256set×16way×1b，16=2way×8组)，含 extra_reset。
//       带 ridx 折叠选择逻辑。
//   FoldedSRAMTemplate_1 : 逻辑 512set×2way×12b(bp_tage)，width=1（无折叠）→
//       直通内层 SplittedSRAMTemplate_4 (512set×2way×struct{valid,tag,ctr})。
// =============================================================================
...
// =============================================================================
//   FoldedSRAMTemplate_1 : width=1 → 无折叠，纯直通到 SplittedSRAMTemplate_4。
// =============================================================================
