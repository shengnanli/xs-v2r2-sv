...
> 验证：`verif/ut/Composer/`（UT 双例化逐拍比对 + Formality 等价）
...
5 预测器 + DelayN 两侧同名黑盒（`hdlin_unresolved_modules=black_box`）。结果：
...
```
cd verif/ut/Composer && make fm   # FM_RESULT 含 20 个假阳性，已逐拍探针证伪
```
