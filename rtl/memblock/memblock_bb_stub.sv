// MemBlock assembly-mode 黑盒 stub（34 类：20 green-305 SIGNOFF_PASS + 14 aux-PASS child）。
// 端口方向自 golden 提取；ref/impl 两侧共读此 stub → 这些 child 成对称显式端口黑盒。
// ~15 glue child（HPerfMonitor_3/PFEvent/Mbist*/DelayN*/PipelineRegModule*/FrontendBridge）
// 不在此 stub —— 改为两侧读入 golden RTL 真实 elaborate（见 fm_eq_bb.tcl 的 GLUE closure）。
// 生成: verif/ut/MemBlock 手工从 memblock_stub.sv 滤除 glue 类；勿手改 stub 端口。
