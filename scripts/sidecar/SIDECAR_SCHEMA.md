# FM sidecar schema + 证明契约(2026-07-19, v5 五审版)
...
| `objects` | `{blackbox, interface_only, unmatched}`,各为 **pair-triple 列表** `[{id, ref_path, impl_path}]`(id token 排序唯一;ref_path 前缀 `r:/WORK/<top>`、impl_path 前缀 `i:/WORK/<top>` namespace 校验;拒 Unicode Cc/Cf) |
...
## 对象映射(五审: pair-triple, 配对关系参与判定)

emitter **不做**模块名归一(多实例会折叠)。native facts 与 envelope 的 `objects` 三类都保存
**实际配对三元组** `[{id, ref_path, impl_path}]`(raw path;非两个独立集合——独立集合无法拒
交叉配对 r0→i1/r1→i0)。expectation.allow 同格式。validator 按**三元组逐项精确相等**判定
(id 与两侧路径的配对关系都参与):交叉配对、interface_only 错 impl_path、namespace 互换
(ref 用 i:/ 或 impl 用 r:/)、错误 top 的路径全部被拒。

## 安全读取(五审: TOCTOU 消除)

native/envelope/expectation 一律 `os.open(O_NOFOLLOW|O_CLOEXEC)` 单次打开 + 同 fd `fstat`
...
7. 模式:
   ...
   - **assembly**: ①数量对称 `compare_ref==compare_impl ∧ bbout_ref==bbout_impl`
     ②数量-对象证据一致(`bool(count)==bool(objects_ref)==bool(objects_impl)`)
     ③映射精确(四审): sidecar raw 路径集 == allow 映射集, 按类别——
       `objects.blackbox_ref == sorted(allow.blackbox[].ref_path)` 且 impl 同理;
       `objects.unmatched_ref/impl == sorted(allow.unmatched[].ref_path/impl_path)`;
       `objects.interface_only == sorted(allow.interface_only[].ref_path)`(非子集,类别不可互换)
     ④qualifications 全空 —— 全部满足才 SUCCEEDED,否则 PARTIAL。
     (对象对称性由映射精确性隐含: ref/impl 集都钉到同一映射, 5(0)数量已被①拒。)
