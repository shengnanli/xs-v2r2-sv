#!/usr/bin/env python3
"""
IssueQueueFaluFmac / EntriesFaluFmac 生成器(从 gen_iq_acfd.py 派生)。

可读核手写在 rtl/backend/{EntriesFaluFmac,IqEntryFfmac}.sv + iq_ffmac_pkg.sv,
本脚本只生成「flat golden 端口 ↔ 可读核 struct 端口」的 wrapper 与双例化 tb:

  Entries:
    EntriesFaluFmac_wrapper.sv  —— golden 同名扁平 wrapper(例化可读核),FM/UT 用。
    verif/ut/IssueQueueFaluFmac/entries_tb.sv —— 双例化(u_g golden vs u_i wrapper)
        逐拍比对全部输出。单条目两侧:u_g 用 golden EnqEntry_12/OthersEntry_120/122;
        u_i 用可读核 xs_iq_entry_ffmac。EnqPolicy_8/UIntCompressor 两侧共享 golden 黑盒。

★ 与 AluCsrFenceDiv 变体的差异(本变体大幅简化)★
  numEntries 18(2/2/14)、numDeq 1、numRegSrc 3;唤醒 fpWen;无 loadDependency /
  RegCache / imm / og1Resp.bits / 双端口。og0Cancel 仅 [8] 被引用。
...
BK = XSSV / "rtl/backend"
UT = XSSV / "verif/ut/IssueQueueFaluFmac"
GEN_HDR = "// 自动生成:scripts/gen_iq_ffmac.py —— 勿手改\n"

# ---- 维度参数(与 iq_ffmac_pkg 一致)----
NUM_ENTRIES, NUM_ENQ, NUM_SIMP, NUM_COMP, NUM_DEQ = 18, 2, 2, 14, 1
...
# ----------------------------------------------------------------------------
# entry_t 字段 ↔ golden flat 字段后缀 的映射(FaluFmac:无 imm/ld_dep/regcache;有 fpu)
# ----------------------------------------------------------------------------
...
def gen_entries_wrapper(ports):
    ...
    for d, w, n in ports:
        ...
        if m:
            # golden deqEntry 的 psrc/srcType 写作 'statuspsrc'/'statussrcType'(无下划线分隔)
            suf = m.group(2)
            suf = re.sub(r"status(psrc|srcType)", r"status_srcStatus_0_\1", suf) \
                if re.match(r"status(psrc|srcType)$", suf) else suf
            unpack.append(f"  assign {n} = {entry_field_expr(f'c_deq_entry', suf)};"); continue
...
def emit_entries_wrapper():
    ports = parse_ports("EntriesFaluFmac", "EntriesFaluFmac.sv")
    pack, unpack = gen_entries_wrapper(ports)
    out = [GEN_HDR]
    out.append("// EntriesFaluFmac golden 同名扁平 wrapper —— 例化可读核 xs_EntriesFaluFmac_core。\n")
    out.append("module EntriesFaluFmac import iq_ffmac_pkg::*; (\n")
    ...
    # 整组 enq_bits 先清 0,再按存在的端口逐字段覆盖,避免悬空成 X。
    out.append("    c_enq_bits = '0;\n")
    out.append("    c_wk_iq = '0; c_wk_iq_d = '0; c_wk_wb = '0; c_wk_wb_d = '0;\n")
    out.extend(p + "\n" for p in pack)
    out.append("  end\n")
    out.extend(u + "\n" for u in unpack)
    out.append("  xs_EntriesFaluFmac_core u_core (\n")
    out.append(ENT_CORE_PORTS)
    out.append("  );\nendmodule\n")
    (BK / "EntriesFaluFmac_wrapper.sv").write_text("".join(out))
    print("wrote EntriesFaluFmac_wrapper.sv  (ports=%d pack=%d unpack=%d)"
...
def emit_entries_tb():
    ...
    L = []
    L.append("// 自动生成:scripts/gen_iq_ffmac.py(entries tb)—— 勿手改\n")
    ...
    def inst(mod, pfx):
        ...
        s.append("\n  );\n")
        return "".join(s)
    L.append(inst("EntriesFaluFmac", "g"))
    L.append(inst("EntriesFaluFmac_xs", "i"))
...
def emit_entries_xs_variant():
    txt = (BK / "EntriesFaluFmac_wrapper.sv").read_text()
    txt = txt.replace("module EntriesFaluFmac ", "module EntriesFaluFmac_xs ")
