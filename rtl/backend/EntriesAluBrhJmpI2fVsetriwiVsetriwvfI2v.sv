// =============================================================================
// xs_EntriesAluBrhJmpI2fVsetriwiVsetriwvfI2v_core —— 香山 V2R2(昆明湖) 发射队列
//   条目阵列 可读重写 (AluBrhJmpI2fVsetriwiVsetriwvfI2v 变体, hasCompAndSimp 路, IQ 收官)
// -----------------------------------------------------------------------------
// 设计源:src/main/scala/xiangshan/backend/issue/Entries.scala
// golden 对照:EntriesAluBrhJmpI2fVsetriwiVsetriwvfI2v.sv(例化 2×EnqEntry_4 +
//             6×OthersEntry_44(simp) + 16×OthersEntry_50(comp) + 2×EnqPolicy 转移策略)
//
// ★ 本模块 = 条目阵列 + 转移策略 + 年龄最老选择 ★(结构与 AluMulBkuBrhJmp 样板逐行同构)
// 相对样板 EntriesAluMulBkuBrhJmp 的差异**全部下沉到单条目核**(FuType 保留位 + payload
// 携带 fp/vec/fpu 字段),阵列层(转移策略 / 年龄选择 / 阵列汇聚 / enq 延迟唤醒辅助寄存器 /
// issueResp 生成)与样板**完全一致**:
//   - issueResp:resps=[og0=block, og1=success, 0, 0],resp 纯由 issueTimer 决定;
//     valid 按 deqPortIdx 选 og0/og1 端口(timer0→og0[port], timer1→og1[port])。
//
// X 铁律:Mux1H/oldest 选择用「sel[i] ? entry[i] : 0」累加(OR),sel 为 0 时不引 X。
// =============================================================================
module xs_EntriesAluBrhJmpI2fVsetriwiVsetriwvfI2v_core import iq_abjivvi_pkg::*; (
  input  logic clock,
  input  logic reset,
  ...
  // ===========================================================================
  // 1. issueRespVec:resps(issueTimer)(deqPortIdx)。resps=[og0,og1,0,0],
  ...
  generate
    for (e = 0; e < NUM_ENTRIES; e++) begin : g_entry
      ...
      xs_iq_entry_abjivvi #(.IS_ENQ(IS_ENQ_E), .IS_COMP(IS_COMP_E)) u_ety (
        .clock          (clock),
        .reset          (reset),
