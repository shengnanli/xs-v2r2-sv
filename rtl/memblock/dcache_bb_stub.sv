// ============================================================================
// dcache_bb_stub.sv —— DCache assembly 边界: 仅 10 个已证 305 SIGNOFF_PASS 子模块
// 的显式端口黑盒 stub (两侧同读, identity 配对, depends_on 断言其等价证明).
// ----------------------------------------------------------------------------
// 这些子模块在独立 305 target 已单独证明 SUCCEEDED/PASS_DEAD_REF (class=SIGNOFF_PASS):
//   BankedDataArray / L1CohMetaArray(metaArray) / L1FlagMetaArray(errorArray) /
//   L1FlagMetaArray_1(accessArray) / DuplicatedTagArray(tagArray) /
//   LoadPipe(ldu_0/1/2) / MainPipe(mainPipe) / MissQueue / ProbeQueue /
//   WritebackQueue(wb)
// DCache 层只证集成 glue 与非 305 逻辑子(CtrlUnit/Arbiter*/TreeArbiter/MissReadyGen/
//   PrefetcherMonitor/FDPrefetcherMonitor/BloomFilter/CounterFilter/
//   L1PrefetchSourceArray/StorePipe) 的 live logic —— 后者两侧 elaborate 真逻辑,
//   不在本 stub.
// ============================================================================
