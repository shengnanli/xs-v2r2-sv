...
## Frontend(✅ 完成)
分支预测(Predictor/Composer/TAGE-SC/ITTage/FTB/FauFTB/RAS)、取指(NewIFU/ICache 全族/IPrefetchPipe/WayLookup/PreDecode/RVCExpander/IBuffer/InstrUncache)、Ftq(112955→2115 行)、Frontend 顶层。全部 UT 验证(多种子逐拍 0 错);FM:叶子/中小模块 SUCCEEDED,**8 个大模块为部分验证**(Composer/Ftq/IBuffer/ICacheMissUnit/IPrefetchPipe/NewIFU/Tage_SC 末次 verify FAILED——failing=20 为默认上限截断、另有数百至数千 unverified 点未验,已报告 failing 均逐点探针证伪;Frontend 顶层 FAILED 仅 1 failing 已证伪、0 unverified),以 UT 为权威,详见各模块 doc。
...
> **MemBlock FM 如实口径**:全部模块 UT 多种子逐拍 0 错为权威。FM 分三类:① 叶子/中小模块 SUCCEEDED;② **大模块部分验证**(BankedDataArray/LoadUnit/LoadQueue/LoadQueueReplay/LoadQueueUncache/LqExceptionBuffer/LoadMisalignBuffer/LsqWrapper/MainPipe/MissQueue/Sbuffer/StoreQueue/StoreExceptionBuffer/StoreMisalignBuffer/Uncache/TLBNonBlock/PTW/PtwCache/LLPTW 末次 verify **FAILED**——failing=20 为 Formality 默认 `verification_failing_point_limit=20` 截断,另有大量 unverified 点未验(如 PtwCache 71912/LoadQueueReplay 17141),已报告 failing 均逐点探针/源级证伪);③ **未跑成**(StorePipe:golden 空壳 FM-081 NOT RUN;L2TLB/L2TlbMissQueue:golden `ram_40x47` OOB lint 致 ref link 失败,verify 未执行)。DCacheWrapper/L2TLBWrapper/MemBlock 顶层的 SUCCEEDED 为脚本对 perf 假阳性 waive 后判定(原始 verify FAILED),详见各模块 doc。
...
| 控制聚合 | **CtrlBlock**(41426行,9轮;重定向流水/decode-buffer FSM/写回压缩/快照 flushVec 前缀OR;22 子模块黑盒;UT 1736 输出 seed1/7/42 各200k/0;FM 实际 **FAILED**:72701 passing / 20 failing / 14753 unverified(83% 截断中止),前 20 failing 含 17 个本核 glue 寄存器(decodeBufValid/ftqCommitValid 等)未逐点证伪——UT 为权威,FM 部分验证,见 CtrlBlock.md §10) |
...
- **Backend 顶层**(capstone,1230 端口)✅ **UT 三种子 200k errors=0 通过(收官)**
  ...
  - **FM 诚实**:`make fm` 结果 **FAILED**(192416 passing / 20 failing / 1959 unverified;failing=20
    恰为 Formality 默认 verification_failing_point_limit=20 截断,verify 在 98% 处提前中止,尚有
    1959 点未验证)。**前 20 个** failing **全部在子模块
    内部**(inner_ctrlBlock/* 的 wbData vls_isVlm·isWhole·rename lsrc / inner_wbDataPath/* 的
    WbArbiter io_in_ready),**Backend 自身 glue 在前 20 中 0 failing**(该论证只覆盖前 20 个);
    根因是 FM 把更深叶子(Rob/Rename/
...
- **XSCore 顶层**(golden 12660 行,285 功能端口)✅ **UT 三种子 120k errors=0 通过**
  ...
  - **FM 诚实**:`make fm` 结果 **FAILED**(645091 passing / 20 failing / 10379 unverified;failing=20
    恰为 Formality 默认 verification_failing_point_limit=20 截断,verify 在 98% 处提前中止,尚有
    10379 点未验证)。**前 20 个** failing **全部在
    memBlock 子模块内部**(inner_xbar/inner_buffers 的 TileLink auto_in_* data/param 引脚),
    **XSCore 自身 top glue 在前 20 中 0 failing**(该论证只覆盖前 20 个);根因 FM 把更深 TL
    叶子黑盒后 ref 扇入悬空无法配对
    (工具自己明示 "unmatched undriven signals in reference fan-in",同 Backend 同类局限)。
    BEU glue 寄存器均 matched+verified。**UT 120k bit-exact 为权威,FM 为部分验证**。
...
- **FM**(best-effort):`make fm-L2Top` 结果 **FAILED**(38707 passing / 20 failing / 7 unverified;
  failing=20 恰为 Formality 默认 verification_failing_point_limit=20 截断,verify 提前中止)。
  **前 20 个 failing compare point 均在 `inner_l2cache/`
  (TL2CHICoupledL2)内部**(perf 寄存器/slices error_bits_address/txreq_arb 等),**L2Top 顶层 glue 在前 20 中 0 failing**(该论证只覆盖前 20 个);
  根因同 XSCore——FM 把 6469 行巨型子模块完整读入两侧而非黑盒,其内部 undriven 输入扇入无法配对(FM ATTENTION
  确认 unmatched undriven fan-in)。以 120k×3 UT bit-exact 为权威,FM 为部分验证。
- 文档 docs/l2/L2Top.md。
...
> (0/182 输出逐拍一致)。④ FM FAILED(38707/20/7,20 为截断上限):前 20 个 failing 点全在 inner_l2cache 内部、顶层 glue 在前 20 中 0 failing,UT 为权威、FM 部分验证。

## Slice L2 cache slice 主体装配(✅ 完成,rtl/l2/)
L2 cache **一个 bank** 的完整流水装配(RequestArb/MainPipe/Directory/DataStorage/MSHRCtl 等装成一条 cache 流水)。
- golden `Slice.sv` 10019 行,465 端口,**18 实例/18 类型黑盒**:`RequestArb`(仲裁)/`MainPipe_1`(主流水)/
  `Directory`(目录)/`DataStorage`(数据阵列)/`MSHRCtl`(MSHR 控制,含 16 MSHR)/`RequestBuffer`/`SinkA`/`SinkC`/
  `GrantBuffer`/`TXREQ·TXDAT·TXRSP`/`RXSNP·RXDAT·RXRSP`/`MSHRBuffer`×2(refill/release 缓冲)/`L2Slice`(MBIST)。
- golden 实测**无 `_T_<n>`/`_GEN_<n>` 匿名临时名**;唯一触闸门的是 perf 第二级 RegNext `io_perf_<N>_value_REG_1`
  (`_REG_<数字>`),**具名重写为两级流水 `perf_<N>_value_s1`→`perf_<N>_value_s2`**(只喂 io_perf 输出,不被引脚引用,
  改名安全)。顶层 glue 四块:(1) 9 条 MBIST bore 改名喂 mbistPl;(2) **唯二时序 glue**=两个 always(异步复位
  releaseBufResp_s3_valid/io_error_valid 打拍 + 同步 error_bits 打拍 + 11 路 perf 各 2 级 RegNext);(3) 4+1 条
  dontTouch `_probe` 探针(死端);(4) 24 条 io 输出 assign(`io_dirResult_valid` 唯一带运算的与门控,余 io 由子模块
  引脚直驱)。
- 产物:可读核 `rtl/l2/Slice.sv`(xs_Slice_core)+ slice_pkg.sv + ports/decls/**glue**/inst/outassign svh +
  Slice_wrapper.sv + slice_blackbox_stubs.sv;生成器 `scripts/gen_slice.py`(派生 gen_l2top.py,新增 perf 二级流水
  具名重写 + 套壳闸门自检)。
- **套壳闸门**(`_T_<n>`/`_GEN_`/`_REG_<编号>`/`io_*_n_m`/`RANDOMIZE`,去注释)= **0**(全 7 个源文件)。
- **UT(权威)**:-f 闭包 **82 模块/82 文件**(含 Directory→SubDirectory→SRAM/MainPipe/MSHRCtl→MSHR 全叶子,
  3.2 MB,编译 ~4s,0 错)。**双例化逐拍比 260 输出:seed1/7/42 各 120000 拍 errors=0,distinct_diverging_ports=0/260**。
- **FM(子模块两侧黑盒,纯比对装配层)**:ref=golden Slice.sv 单文件、impl=核+wrapper,18 子模块全 black_box,
  **35733/35734 compare point Passing**,唯一 1 个 Failing 是 BBPin `reqBuf/io_hasMergeA`(驱动 golden dontTouch 死网
  `_probe`,ref 侧 undriven cut-point 的固有伪差),**顶层 glue 0 failing**。
- 文档 docs/l2/Slice.md。

> **诚实**:① 18 子模块实例全 golden 黑盒,本层只做装配互联 + glue。② `_GEN_/_T_/_REG_<编号>/RANDOMIZE` 实测核+
> 5 个 svh 去注释后全 0(perf 二级 RegNext 已具名重写,非 golden 套壳)。③ **UT 三种子 120k errors=0 已确证**
> (0/260 输出逐拍一致)。④ FM 仅 1 failing = `_probe` dontTouch 死网伪差(任务已预告 Slice 带 `_probe`、不可 ST
> 替换),顶层 glue 0 failing。⑤ Slice_1/2/3 仅差 sliceId+子模块变体名(diff 12 行),本轮聚焦基准 Slice;Slice_4
> 是 OpenLLC home-node slice(不同模块),不在本轮。
...
| **MSHRCtl** | 14467 | 控制阵列 glue+仲裁(1001 端口,16 MSHR + 选择器 + SourceB + 4 仲裁器**全黑盒**) | ✅ 各 **200k errors=0 + ierr=0**(160.8M 输出比,804 输出 + 3 glue 探针) | ✅ **26444 passing / 0 failing,SUCCEEDED** | MSHR 资源池:分配(选择器 one-hot→OHToUInt)/容量背压(mshrFull 留 1 槽给 SinkB)/响应按 mshrId·PA 路由/nestedwb 广播/4 把 fastArb 出口/perf。generate-for 单模板装 16 MSHR;★坑:hpm_timers 异步 reset 漏掉则 UT 过但 FM 判 20 寄存器不等价★ |
...
| 级别 | 状态 | 说明 |
|------|------|------|
| **UT 单元级** | ✅ 156 模块 | VCS 双例化逐拍比对 golden(全部多种子 0 错,为权威)+ Formality 等价比对(多数 SUCCEEDED;前端/访存 ~30 个大模块为部分验证:FAILED、failing=20 截断+unverified 未验、已报告 failing 逐点证伪;StorePipe/L2TLB/L2TlbMissQueue FM 未跑成;RenameBuffer FM 被中止无结果) |
