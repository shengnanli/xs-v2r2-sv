...
module L1FlagMetaArray_xs import xs_l1metaarray_pkg::*; (
  input clock,
  input reset,
  ...
  always_comb begin
    ...
    wdata[0] = io_write_0_bits_flag;
  end
  xs_L1FlagMetaArray_core #(.READ_PORTS(4), .WRITE_PORTS(1)) u_core (
...
// 自动生成：scripts/gen_l1metaarray.py —— 勿手改
// L1 元数据阵列 golden 同名 / xs 变体扁平包装：扁平端口 <-> 可读核数组端口。
module L1FlagMetaArray_1_xs import xs_l1metaarray_pkg::*; (
  input clock,
  input reset,
  input io_read_0_valid,
  input [7:0] io_read_0_bits_idx,
