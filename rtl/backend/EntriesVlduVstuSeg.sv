// =============================================================================
// xs_EntriesVlduVstuSeg_core —— 香山 V2R2(昆明湖) 发射队列条目阵列 可读重写
//                            (VlduVstuVseglduVsegstu 变体, hasCompAndSimp 路)
// ★ 与 EntriesVlduVstu_core 的唯一差异:单条目改例化 xs_iq_entry_vlduvstuseg
//   (status 多 fu_type_vsegldu/vsegstu 两位 FU one-hot,随条目透传);其余逻辑完全一致 ★
// -----------------------------------------------------------------------------
...
// =============================================================================
module xs_EntriesVlduVstuSeg_core import iq_vlduvstuseg_pkg::*; (
  input  logic clock,
  input  logic reset,
  ...
  //   与绿姊妹 EntriesVlduVstu 同修法(h0/h1/h2 提升到模块级组合网)。
  ...
  always_comb begin
    for (int e = 0; e < NUM_ENTRIES; e++) begin
      ...
      if (tmr == 2'h0) begin
        ...
        // timer 饱和窗口:三路 mem 响应按索引匹配(命中已在上方组合网算好)。
        v  = h0 | h1 | h2;
  ...
  generate
    for (e = 0; e < NUM_ENTRIES; e++) begin : g_entry
      ...
      xs_iq_entry_vlduvstuseg #(.IS_ENQ(IS_ENQ_E), .IS_TRANS(IS_TRANS_E)) u_ety (
        .clock          (clock),
        .reset          (reset),
