// ============================================================================
// DCache 簇 IT testbench —— 基于 per-module DCache UT tb (gen_dcache.py 生成),
//   u_i 由 DCache_xs(UT, 周围 golden stub) 改为 DCache_it(IT, 周围全部重写 _xs 叶子)。
//   逐拍比对 golden DCache(u_g) 全部顶层功能输出; +NCYCLES 运行时覆盖拍数;
//   协议感知激励(合法 load/store + refill/probe 握手)穿透 MainPipe/MissQueue;
//   activity 计数(load/store fire, miss A 发出, refill D, probe B) 报激励穿透度;
//   非功能口(DFT bore 链 / 重写主动置零的 perf) 单列 nonfunc_errors, 不计入 PASS。
// ============================================================================
...
module tb;
  ...
  // ---- activity counters (激励穿透度) ----
  longint act_load_fire = 0, act_sta_fire = 0, act_store_fire = 0, act_atomic_fire = 0;
  longint act_miss_a = 0, act_refill_d = 0, act_probe_b = 0, act_release_c = 0;
  longint act_load_resp = 0, act_load_miss = 0;
  // ---- 协议感知激励工作集: 小集合(16 组 x 小 tag 集) -> refill 后再命中 ----
  logic [7:0]  ws_set0, ws_set1, ws_set2;   // 256 组中取低 16 组循环
  logic [33:0] ws_tag0, ws_tag1, ws_tag2;   // 小 tag 集 -> 同组多路 -> 触发替换
  always #5 clk = ~clk;
  ...
  always @(negedge clk) begin
    if (rst) begin
      ...
      // 工作集滚动: blocking 先更新, 供本拍地址 nonblocking 引用。
      // 低 4 位组号(16 组)+ 2 位 tag(4 路 tag) -> 同组易满 4 路 -> 触发 replace/miss。
      ws_set0 = {4'h0, $urandom_range(0,15)};
      ws_set1 = {4'h0, $urandom_range(0,15)};
      ws_set2 = {4'h0, $urandom_range(0,15)};
      ws_tag0 = {32'h0, $urandom_range(0,3)};
      ws_tag1 = {32'h0, $urandom_range(0,3)};
      ws_tag2 = {32'h0, $urandom_range(0,3)};
      ...
      // TL 通道: refill/probe 握手偏活跃(穿透 MissQueue/ProbeQueue)。
      auto_client_out_a_ready <= ($urandom_range(0,3) != 0);  // L2 收 miss req
      auto_client_out_b_valid <= ($urandom_range(0,3) == 0);  // L2 发 probe
      ...
      auto_client_out_c_ready <= ($urandom_range(0,3) != 0);  // L2 收 release
      auto_client_out_d_valid <= ($urandom_range(0,2) != 0);  // L2 发 refill/grant
      ...
      // ---- 协议感知激励: load 路 (3 路) ----
      // valid 偏高(穿透流水), kill 偏低(不轻易杀), cmd=load(M_XRD=0),
      // 地址收敛到小工作集(共享 32 行) -> refill 后命中 + 触发 miss/replace。
      io_lsu_load_0_req_valid <= ($urandom_range(0,3) != 0);
      io_lsu_load_0_req_bits_cmd <= 5'b0;  // M_XRD: 普通 load
      io_lsu_load_0_req_bits_vaddr <= {36'h0, ws_set0, 6'h0};
      io_lsu_load_0_req_bits_vaddr_dup <= {36'h0, ws_set0, 6'h0};
      ...
      io_lsu_load_0_s1_kill <= ($urandom_range(0,7) == 0);
      io_lsu_load_0_s2_kill <= ($urandom_range(0,7) == 0);
      io_lsu_load_0_is128Req <= $urandom_range(0,1);
      io_lsu_load_0_pf_source <= 3'($urandom);
      io_lsu_load_0_s1_paddr_dup_lsu    <= {ws_tag0, ws_set0, 6'h0};
      io_lsu_load_0_s1_paddr_dup_dcache <= {ws_tag0, ws_set0, 6'h0};
      io_lsu_load_1_req_valid <= ($urandom_range(0,3) != 0);
      io_lsu_load_1_req_bits_cmd <= 5'b0;
      io_lsu_load_1_req_bits_vaddr <= {36'h0, ws_set1, 6'h0};
      io_lsu_load_1_req_bits_vaddr_dup <= {36'h0, ws_set1, 6'h0};
      ...
      io_lsu_load_1_s1_kill <= ($urandom_range(0,7) == 0);
      io_lsu_load_1_s2_kill <= ($urandom_range(0,7) == 0);
      io_lsu_load_1_is128Req <= $urandom_range(0,1);
      io_lsu_load_1_pf_source <= 3'($urandom);
      io_lsu_load_1_s1_paddr_dup_lsu    <= {ws_tag1, ws_set1, 6'h0};
      io_lsu_load_1_s1_paddr_dup_dcache <= {ws_tag1, ws_set1, 6'h0};
      io_lsu_load_2_req_valid <= ($urandom_range(0,3) != 0);
      io_lsu_load_2_req_bits_cmd <= 5'b0;
      io_lsu_load_2_req_bits_vaddr <= {36'h0, ws_set2, 6'h0};
      io_lsu_load_2_req_bits_vaddr_dup <= {36'h0, ws_set2, 6'h0};
      ...
      io_lsu_load_2_s1_kill <= ($urandom_range(0,7) == 0);
      io_lsu_load_2_s2_kill <= ($urandom_range(0,7) == 0);
      io_lsu_load_2_is128Req <= $urandom_range(0,1);
      io_lsu_load_2_pf_source <= 3'($urandom);
      io_lsu_load_2_s1_paddr_dup_lsu    <= {ws_tag2, ws_set2, 6'h0};
      io_lsu_load_2_s1_paddr_dup_dcache <= {ws_tag2, ws_set2, 6'h0};
      io_lsu_sta_0_req_valid <= ($urandom_range(0,3) != 0);
      io_lsu_sta_1_req_valid <= ($urandom_range(0,3) != 0);
      io_lsu_store_req_valid <= ($urandom_range(0,2) != 0);
      io_lsu_store_req_bits_vaddr <= {36'h0, ws_set0, 6'h0};
      io_lsu_store_req_bits_addr <= {ws_tag0, ws_set0, 6'h0};
  ...
  // ---- activity counters: 统计 golden(u_g) 簇内真实活跃事件, 报激励穿透度 ----
  always @(posedge clk) if (!rst) begin
    // load 流水 fire (req_valid & req_ready)
    if (io_lsu_load_0_req_valid && g_io_lsu_load_0_req_ready) act_load_fire++;
    if (io_lsu_load_1_req_valid && g_io_lsu_load_1_req_ready) act_load_fire++;
    if (io_lsu_load_2_req_valid && g_io_lsu_load_2_req_ready) act_load_fire++;
    // sta 请求 valid (golden StorePipe 无 ready 输出, 计 valid)
    if (io_lsu_sta_0_req_valid) act_sta_fire++;
    if (io_lsu_sta_1_req_valid) act_sta_fire++;
    // store / atomics fire
    if (io_lsu_store_req_valid   && g_io_lsu_store_req_ready)   act_store_fire++;
    if (io_lsu_atomics_req_valid && g_io_lsu_atomics_req_ready) act_atomic_fire++;
    // load resp / miss
    if (g_io_lsu_load_0_resp_valid) act_load_resp++;
    if (g_io_lsu_load_1_resp_valid) act_load_resp++;
    if (g_io_lsu_load_2_resp_valid) act_load_resp++;
    if (g_io_lsu_load_0_resp_valid && g_io_lsu_load_0_resp_bits_miss) act_load_miss++;
    if (g_io_lsu_load_1_resp_valid && g_io_lsu_load_1_resp_bits_miss) act_load_miss++;
    if (g_io_lsu_load_2_resp_valid && g_io_lsu_load_2_resp_bits_miss) act_load_miss++;
    // TL 通道握手: A(miss req out), D(refill in), B(probe in), C(release out)
    if (g_auto_client_out_a_valid && auto_client_out_a_ready) act_miss_a++;
    if (auto_client_out_d_valid   && g_auto_client_out_d_ready) act_refill_d++;
    if (auto_client_out_b_valid   && g_auto_client_out_b_ready) act_probe_b++;
    if (g_auto_client_out_c_valid && auto_client_out_c_ready) act_release_c++;
  end
  ...
  initial begin
    ...
    $display("ACTIVITY load_fire=%0d sta_fire=%0d store_fire=%0d atomic_fire=%0d",
             act_load_fire, act_sta_fire, act_store_fire, act_atomic_fire);
    $display("ACTIVITY load_resp=%0d load_miss=%0d miss_A=%0d refill_D=%0d probe_B=%0d release_C=%0d",
             act_load_resp, act_load_miss, act_miss_a, act_refill_d, act_probe_b, act_release_c);
