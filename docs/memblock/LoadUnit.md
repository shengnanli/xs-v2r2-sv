...
> **实现状态**：S0、S1 已按 Scala 完整重写并经 UT 逐拍比对通过（见下）。S2/S3 的写回/回灌/
> feedback/rollback 输出尚以 0 占位（`LoadUnit_core_body.svh` 末尾 TODO 段），perf 计数中
> S0/S1 相关项（0~5）已实现、perf_6（s2 dcache_miss）待补。数据扩展所需的纯函数
> （`gen_rdata_oh`/`new_rdata`/`genDataSelectByOffset` 等）已在 pkg 中实现，S2/S3 续写时直接复用。

## 6. 验证

- UT：golden `LoadUnit` 与可读核 `LoadUnit_xs` 双例化，随机激励逐拍比对全部 418 路输出。
  - 种子 1 / 7：S0/S1 所有输出 **逐拍一致**（无 S0/S1 区失配）。
  - 种子 42：S0/S1 仅在复位后头两拍（首两条 load）的 s1 异常/kill 路有瞬态差异（reset 区，
    随后清零），其余一致。
  - S2/S3 占位输出按预期失配（尚未实现）。
- FM：因 S2/S3 占位 + 可读核与 golden 结构差异大，签名分析无法整体配对（`FAILED/INCONCLUSIVE`）。
  与 REWRITE_STYLE 一致，正确性以 UT 为主、FM 暂不可判，待 S2/S3 补齐后再评估。
...
| `rtl/memblock/LoadUnit_core_body.svh` | 可读核主体（S0 全 + S1 全 + S2/S3 占位） |
