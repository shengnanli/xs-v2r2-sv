...
// =============================================================================
  ...
  wire firstWithMisalign  = ent[rdataPtrExt[0].value].unaligned;
  wire firstWithCross16B   = ent[rdataPtrExt[0].value].cross16Byte;
  ...
  assign io_maControl_toStoreMisalignBuffer_uop_uopIdx     = uop[rdataPtrExt[0].value].uopIdx;
  assign io_maControl_toStoreMisalignBuffer_uop_robIdx_flag= uop[rdataPtrExt[0].value].robIdx_flag;
  assign io_maControl_toStoreMisalignBuffer_uop_robIdx_value=uop[rdataPtrExt[0].value].robIdx_value;
  ...
  wire [255:0] Cross16ByteData = {128'b0, sd_rdata_data[0]} << ({1'b0,cross16ByteAddrLow4} << 3);
  ...
  wire mmio0 = ent[rdataPtrExt[0].value].mmio;
  wire mmio1 = ent[rdataPtrExt[1].value].mmio;
  wire nc0   = ent[rdataPtrExt[0].value].nc;
  wire nc1   = ent[rdataPtrExt[1].value].nc;
  wire mmioStall0 = mmio0;
  wire mmioStall1 = mmio1 | mmio0;
  wire ncStall0   = nc0;
  wire ncStall1   = nc1 | nc0;
  wire exc0 = ent[rdataPtrExt[0].value].hasException;
  wire exc1 = ent[rdataPtrExt[1].value].hasException
              | (ent[rdataPtrExt[0].value].hasException
                 & (uop[rdataPtrExt[1].value].robIdx_flag == uop[rdataPtrExt[0].value].robIdx_flag)
                 & (uop[rdataPtrExt[1].value].robIdx_value == uop[rdataPtrExt[0].value].robIdx_value));
  ...
  wire vec0   = ent[dp0].isVec;
  wire vecMb0 = ent[dp0].vecMbCommit;
  wire allv0  = ent[dp0].addrvalid & ent[dp0].datavalid;
  wire vecNotAllMask0 = |sd_rdata_mask[0];
  wire vecHasExcFlag0 = vecExceptionFlag_valid & vec0
                        & (vecExceptionFlag_robIdx_flag == uop[dp0].robIdx_flag)
                        & (vecExceptionFlag_robIdx_value == uop[dp0].robIdx_value);
  wire toSbufferVecValid0 = (~vec0 | (vecMb0 & allv0 & vecNotAllMask0)) & ~exc0 & ~vecHasExcFlag0;
  // misalignToDataBufferValid（首条非对齐跨 16B 的合法出队）
  wire misalignToDataBufferValid = ent[dp0].allocated & ent[dp0].committed
        & ((~vec0 & allv0) | vecMb0)
        & canDeqMisaligned & (~isCross4KPage | isCross4KPageCanDeq | exc0);

  // 第 1 口
  wire [SQ_IDX_W-1:0] dp1 = rdataPtrExt[1].value;
  wire vec1   = ent[dp1].isVec;
  wire vecMb1 = ent[dp1].vecMbCommit;
  wire allv1  = ent[dp1].addrvalid & ent[dp1].datavalid;
  wire vecNotAllMask1 = |sd_rdata_mask[1];
  wire vecHasExcFlag1 = vecExceptionFlag_valid & vec1
                        & (vecExceptionFlag_robIdx_flag == uop[dp1].robIdx_flag)
                        & (vecExceptionFlag_robIdx_value == uop[dp1].robIdx_value);
  ...
  always_comb begin
    ...
    if (firstWithMisalign & firstWithCross16B)
      ...
      db_enq_0_valid = ent[dp0].allocated & ent[dp0].committed
            & ((~vec0 & (allv0 | exc0)) | vecMb0)
            & ~mmioStall0 & ~ncStall0
            & (~ent[dp0].unaligned | (~ent[dp0].cross16Byte & (allv0 | exc0)));
    // payload
    if (canDeqMisaligned & firstWithMisalign & firstWithCross16B) begin
    ...
    end else if (~ent[dp0].cross16Byte & ent[dp0].unaligned) begin
      ...
      db_enq_0_bits_data  = sd_rdata_data[0] << ({1'b0,addrLow4_0} << 3);
  ...
  always_comb begin
    if (firstWithMisalign & firstWithCross16B)
      ...
      db_enq_1_valid = ent[dp1].allocated & ent[dp1].committed
            & ((~vec1 & (allv1 | exc1)) | vecMb1)
            & ~mmioStall1 & ~ncStall1
            & (~ent[dp1].unaligned | (~ent[dp1].cross16Byte & (allv1 | exc1)));
    if (canDeqMisaligned & firstWithMisalign & firstWithCross16B) begin
    ...
    end else if (~ent[dp1].cross16Byte & ent[dp1].unaligned) begin
      db_enq_1_bits_addr  = {pa_rdata[1][PADDR_BITS-1:4], 4'b0};
      db_enq_1_bits_vaddr = {va_rdata[1][VADDR_BITS-1:4], 4'b0};
      db_enq_1_bits_data  = sd_rdata_data[1] << ({1'b0,addrLow4_1} << 3);
  ...
  assign io_sbuffer_0_bits_wline  = db_deq_0_bits_wline;
  ...
  assign io_sbuffer_1_bits_wline  = db_deq_1_bits_wline;
  ...
  wire [1:0] isCboZeroToSbVec = {sbuf1_fire_q & ent[deqPtrExt[1].value].allocated,
                                 sbuf0_fire_q & ent[deqPtrExt[0].value].allocated};
  assign cboZeroToSb = |isCboZeroToSbVec;
  always_ff @(posedge clock) cboZeroFlushSb_q <= cboZeroToSb;
  ...
  always_ff @(posedge clock) begin
    if (reset) begin cboZeroValid <= 1'b0; cboZeroWaitFlushSb <= 1'b0; end
      ...
      if (cboZeroToSb) begin
        ...
        // PriorityMux(isCboZeroToSbVec, deqPtr.map(uop))
        if (isCboZeroToSbVec[0]) begin
          cbz_robF <= uop[deqPtrExt[0].value].robIdx_flag;
          cbz_robV <= uop[deqPtrExt[0].value].robIdx_value;
          cbz_flushPipe <= uop[deqPtrExt[0].value].flushPipe;
          cbz_trigger   <= uop[deqPtrExt[0].value].trigger;
          cbz_dbgEnq <= uop[deqPtrExt[0].value].dbg_enqRsTime;
          cbz_dbgSel <= uop[deqPtrExt[0].value].dbg_selectTime;
          cbz_dbgIss <= uop[deqPtrExt[0].value].dbg_issueTime;
        end else begin
          cbz_robF <= uop[deqPtrExt[1].value].robIdx_flag;
          cbz_robV <= uop[deqPtrExt[1].value].robIdx_value;
          cbz_flushPipe <= uop[deqPtrExt[1].value].flushPipe;
          cbz_trigger   <= uop[deqPtrExt[1].value].trigger;
          cbz_dbgEnq <= uop[deqPtrExt[1].value].dbg_enqRsTime;
          cbz_dbgSel <= uop[deqPtrExt[1].value].dbg_selectTime;
          cbz_dbgIss <= uop[deqPtrExt[1].value].dbg_issueTime;
        end
      end
  ...
  // cboZeroStout exceptionVec 全 0（DontCare → 0）
  `define SQ_CBZ_EV(I) assign io_cboZeroStout_bits_uop_exceptionVec_``I = 1'b0;
