...
> 输入锥而 INCONCLUSIVE（20 个 matched failing），已用探针证明这些点在可达激励下不分歧。
...
- 结果：`FM_RESULT: Verification FAILED or INCONCLUSIVE for LLPTW`。
- 读入/elaborate 通过（无 `FMR_ELAB-147`/`FM-089` 中断）。
- 20 个 **matched failing** DFF：`addr_reg[3..11]`(9)、`enq_ptr_reg[0..2]`(3)、
