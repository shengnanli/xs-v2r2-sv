...
**复核流程(主线必做)**:① 上述结构 grep ② 读核中段样本人工判断 ③ **.svh 套壳检查(见下)** ④ 多种子 UT(seed 1/7/42) ⑤ FM。
任一不达标 → 退回,用"从 Scala 重实现 + 结构硬指标"的强 prompt 重做。

## ⚠️ .svh「机械互联表」套壳闸门(2026-06-17 加强,血的教训)

大互联/集成模块允许把**端口机械互联**拆到 `*.svh`(可读核 + 机械互联表分层),svh 对结构 grep 豁免。**但这只在「模块自身几乎无逻辑、真正逻辑都在被黑盒的子模块里」时合法**(如 MemBlock/DCache/L2TLB 顶层:几十个子模块黑盒,svh 只是例化+连线)。
**致命陷阱**:对**自身含大量组合逻辑**的模块(如 DataPath 的读口仲裁/操作数 Mux、NewDispatch 的路由/排序),若把 golden 模块体直接抽进 `body.svh`,那就是**把 golden 逻辑原文套个壳**——UT/FM 必过(因为就是 golden),但**完全没重写,违反最高准则,拒收**。
(反面教材:DataPath 首版核 136 行只做整数写回,`datapath_body.svh` 3158 个 `_GEN_/_T_` 全是 golden 的 `_intRFReadArbiter_io_in_*_valid_T_1` 读口仲裁逻辑原文;NewDispatch 同病。已回退。)

**硬检查(验收必做)**:`grep -hcE "_GEN_|_T_[0-9]" 模块所有 *.svh`。
- 该数应**接近 0 ~ 几十**(纯例化+连线,给子模块连线偶有少量路由 mux 网)。
- 若**上百~上千**,说明 golden 的**功能逻辑被抽进 svh 而非重写** → 拒收,真正的逻辑必须落在可读核里(struct/enum/function/genvar 表达),svh 只许放子模块例化与端口连线。
