...
ScalarCryptoDecode/FpDecode/VecDecoder/…），共约 721 条 pattern：
...
localparam logic [31:0]  DECODE_MASK  [716];  // (instr & mask) == match 即命中
localparam logic [31:0]  DECODE_MATCH [716];
localparam decode_bits_t DECODE_BITS  [716];  // 命中后取这一行控制位
```
...
手敲 716×15 的表无法可靠校验，照抄 golden 生成名又违规。正确做法是**用脚本从
golden 真值表可靠提取这张数据表**：
