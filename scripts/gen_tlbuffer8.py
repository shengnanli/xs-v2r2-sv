#!/usr/bin/env python3
"""生成 TLBuffer_8 (TL A/D 两通道窄总线 skid 缓冲) 的 wrapper / 变体 / UT / Makefile / 黑盒。

TLBuffer_8 是叶子级结构模块: A/D 两通道各例化一个 Queue2 黑盒。
  · UT 双例化: golden TLBuffer_8 vs 可读 TLBuffer_8_xs, 两侧共用真实 Queue2/ram 叶子,
    随机驱动全部输入、逐拍比对全部输出。
  · FM: ref=golden TLBuffer_8 (Queue 自动黑盒), impl=pkg+可读核+wrapper+黑盒声明。
"""
...
UT = ROOT / "verif" / "ut" / "TLBuffer_8"

TOP = "TLBuffer_8"
CORE = "xs_TLBuffer_8_core"
VARIANT = "TLBuffer_8_xs"
QUEUES = ["Queue2_TLBundleA_14", "Queue2_TLBundleD_15"]
RAMS = ["ram_2x134", "ram_2x77"]
...
def gen_wrapper(ports):
    return ("// 自动生成：scripts/gen_tlbuffer8.py —— 勿手改\n"
...
def gen_blackbox():
    out = ["// 自动生成：scripts/gen_tlbuffer8.py —— 勿手改",
...
def gen_makefile():
    ...
    return f"""# 自动生成：scripts/gen_tlbuffer8.py —— 勿手改
MODULE = TLBuffer_8

RTL_DIR = ../../../rtl
GOLDEN_RTL = ../../../golden/chisel-rtl

RTL_SRCS = $(RTL_DIR)/memblock/tlbuffer8_pkg.sv \\
           $(RTL_DIR)/memblock/TLBuffer_8.sv

TB_SRCS = variants_xs.sv tb.sv

# UT 双例化: 两侧共用真实 Queue2 + ram 叶子。
GOLDEN_SRCS = $(GOLDEN_RTL)/TLBuffer_8.sv \\
              {golden_q} \\
              {golden_r}

WRAPPER_SRCS = $(RTL_DIR)/memblock/TLBuffer_8_wrapper.sv \\
               $(RTL_DIR)/memblock/tlbuffer8_queue_blackbox.sv
FM_VARIANTS = TLBuffer_8
# FM: ref = golden TLBuffer_8.sv (Queue 自动黑盒); impl = pkg + 可读核 + wrapper + 黑盒声明。
...
def main():
    ...
    (RTL / "tlbuffer8_queue_blackbox.sv").write_text(gen_blackbox())
