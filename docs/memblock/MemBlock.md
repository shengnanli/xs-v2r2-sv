...
2. 可读核 `xs_MemBlock_core`（`MemBlock.sv`）提供**架构框架 + 中文讲解**并 `include`
   上述 `.svh`，是学习载体。
3. 全部 71 类子模块作 golden 黑盒（UT/FM 两侧共用同一份 golden 子模块定义）。
...
`MemBlock` + `memblock_stub.sv`（全部 71 类子模块显式端口黑盒）；impl =
...
- **实测结果：`FM_RESULT: Verification SUCCEEDED for MemBlock`**。
- 全部互联 glue 比对点匹配通过；唯一失败点是 **8 路 perf 流水中的第 1、2 路**共 20 个
