...
// ----------------------------------------------------------------------------
  ...
  // ── FM glue-partition 边界收口:rob.io_commits.info 标量→数组桥接 ──────────────
  //   inst.svh 把 rob 黑盒的 io_commits_info_{0..5}_{commitType,ftqIdx_flag,ftqIdx_value,
  //   ftqOffset} 引脚绑到**标量** _rob_io_commits_info_N_*;而 outglue.svh 的 commits→FTQ
  //   流水读的是**数组** _rob_io_commits_info_{flag_commitType,ftqIdx_flag,ftqIdx_value,
  //   ftqOffset}[k]。原实现漏了这层桥接 → 数组悬空(未驱动)、标量引脚未被读。
  //   FM 表现:ref/impl 两侧 rob/io_commits_info_{0..5}_* 共 162 个 BBPin 因「无扇出/未读」
  //   自动配对失败(lane 6/7 因 outglue2 直接读标量而已 matched)。
  //   补齐桥接后:数组被真数据驱动(outglue 读到活的 rob 提交信息),同时 lane 0..5 标量引脚
  //   在两侧都成为「被读」的 BBPin → FM 自动按同名配对为 matched cut-point(与 lane6/7 一致)。
  //   (lane 6/7 沿用 outglue2 的 _rob_io_commits_info_6/7_* 标量读,已 matched,此处不重复。)
  ...
  // commitValid 标量→packed 桥接(outglue s1_isCommit / logic snpt-deq 读 packed [5:0])。
  //   inst.svh 绑 rob 引脚到标量 _rob_io_commits_commitValid_{0..5};读侧用 packed
  //   _rob_io_commits_commitValid[gk]。原实现漏桥接 → packed 悬空。补齐后引脚被读、两侧配对。
  assign _rob_io_commits_commitValid = {
      _rob_io_commits_commitValid_5, _rob_io_commits_commitValid_4,
      _rob_io_commits_commitValid_3, _rob_io_commits_commitValid_2,
      _rob_io_commits_commitValid_1, _rob_io_commits_commitValid_0 };

  // robIdx 标量→packed/索引桥接(logic snpt-deq head 匹配读 flag[gk] / value_N)。
  assign _rob_io_commits_robIdx_flag = {
      _rob_io_commits_robIdx_5_flag, _rob_io_commits_robIdx_4_flag,
      _rob_io_commits_robIdx_3_flag, _rob_io_commits_robIdx_2_flag,
      _rob_io_commits_robIdx_1_flag, _rob_io_commits_robIdx_0_flag };
  ...
  // lane 6/7 别名驱动(snpt-deq head 匹配用;decls 已声明,logic 只读别名):
