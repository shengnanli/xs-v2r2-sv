// 自动生成：gen_l2slice_variants.py —— 勿手改
// L2Slice_3 双例化逐拍比对：golden L2Slice_3 vs 可读重写 L2Slice_3_xs。
`timescale 1ns/1ps
...
module tb;
  ...
  wire [5:0] g_toNextPipeline_0_array;
  wire [5:0] i_toNextPipeline_0_array;
  ...
  wire [5:0] g_toNextPipeline_1_array;
  wire [5:0] i_toNextPipeline_1_array;
  ...
  function automatic logic [5:0] pick_array();
    case ($urandom_range(0, 15))
      ...
      13: return 6'h3c;
      14: return 6'h3d;
      default: return 6'($urandom);  // 全随机：覆盖未命中路径
    endcase
  endfunction
  ...
  initial begin
    ...
    $display("L2Slice_3 checks=%0d errors=%0d", checks, errors);
    if (errors == 0) begin
      $display("TEST PASSED");
      $finish;
    end
