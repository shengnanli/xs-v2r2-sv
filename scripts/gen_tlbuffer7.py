#!/usr/bin/env python3
"""生成 TLBuffer_7 (TL-C 五通道 skid 缓冲) 的 wrapper / 变体 / UT / Makefile / 黑盒声明。

TLBuffer_7 是叶子级结构模块: 5 条 TL 通道 (A/B/C/D/E) 各例化一个 Queue2 黑盒。
  · UT 双例化: golden TLBuffer_7 vs 可读 TLBuffer_7_xs, 两侧共用真实 Queue2/ram 叶子,
    随机驱动全部输入、逐拍比对全部输出 (验证 5 通道方向连线正确)。
  · FM: ref=golden TLBuffer_7 (Queue 自动黑盒), impl=pkg+可读核+wrapper+黑盒声明。
"""
...
UT = ROOT / "verif" / "ut" / "TLBuffer_7"

TOP = "TLBuffer_7"
CORE = "xs_TLBuffer_7_core"
VARIANT = "TLBuffer_7_xs"
QUEUES = ["Queue2_TLBundleA_7", "Queue2_TLBundleD_8", "Queue2_TLBundleB",
          "Queue2_TLBundleC", "Queue2_TLBundleE"]
RAMS = ["ram_2x406", "ram_2x284", "ram_2x309", "ram_2x373", "ram_sink_2x10"]
...
def gen_wrapper(ports):
    return ("// 自动生成：scripts/gen_tlbuffer7.py —— 勿手改\n"
...
def gen_blackbox():
    out = ["// 自动生成：scripts/gen_tlbuffer7.py —— 勿手改",
...
def gen_makefile():
    ...
    return f"""# 自动生成：scripts/gen_tlbuffer7.py —— 勿手改
MODULE = TLBuffer_7

RTL_DIR = ../../../rtl
GOLDEN_RTL = ../../../golden/chisel-rtl

RTL_SRCS = $(RTL_DIR)/memblock/tlbuffer7_pkg.sv \\
           $(RTL_DIR)/memblock/TLBuffer_7.sv

TB_SRCS = variants_xs.sv tb.sv

# UT 双例化: 两侧共用真实 Queue2 + ram 叶子。
GOLDEN_SRCS = $(GOLDEN_RTL)/TLBuffer_7.sv \\
              {golden_q} \\
              {golden_r}

WRAPPER_SRCS = $(RTL_DIR)/memblock/TLBuffer_7_wrapper.sv \\
               $(RTL_DIR)/memblock/tlbuffer7_queue_blackbox.sv
FM_VARIANTS = TLBuffer_7
# FM: ref = golden TLBuffer_7.sv (Queue 自动黑盒); impl = pkg + 可读核 + wrapper + 黑盒声明。
...
def main():
    ...
    (RTL / "tlbuffer7_queue_blackbox.sv").write_text(gen_blackbox())
