// =============================================================================
// iq_ffmacfdiv_pkg —— 香山 V2R2(昆明湖) 发射队列 FaluFmacFdiv 变体 公共类型/参数
// -----------------------------------------------------------------------------
// 设计源:src/main/scala/xiangshan/backend/issue/{Entries,EntryBundles,EnqEntry,
//        OthersEntry,IssueQueue}.scala
// golden 对照:IssueQueueFaluFmacFdiv.sv / EntriesFaluFmacFdiv.sv
//             叶子:EnqEntry_10.sv / OthersEntry_104.sv(simp) / OthersEntry_106.sv(comp)
//
// 发射队列(IssueQueue)是乱序后端的「调度心脏」(总览见 iq_ffmac_pkg 头部)。
//
// 本变体 FaluFmacFdiv = 浮点 IQ,功能单元含 FALU(浮点加减/比较)+ FMAC(浮点乘加)
//   + FDIV/FSQRT(浮点除/开方)。它是 FaluFmac 的「近邻」:多了一个 Fdiv 功能单元,
//   因而**多出一个发射端口(numDeq=2)**——Fdiv 多周期、与 Falu/Fmac 不抢同一条发射通路。
//
// 关键参数(从 golden RTL 端口实测):
//   numEntries = 18, numEnq = 2, numSimp = 2, numComp = 14
//   numDeq     = 2   ★ 与 FaluFmac 的 1 不同 ★
//   numRegSrc  = 3 (浮点乘加最多三个源寄存器 rs1/rs2/rs3)
//   numWakeupFromIQ = 3, numWakeupFromWB = 6   (全部 fpWen-only,纯浮点唤醒;与 FaluFmac 同)
//
// ★ 与 FaluFmac 样板的关键差异(仅四处,唤醒/匹配机制 100% 相同)★
//   1) **numDeq = 2**:dual deqSelOH / og0Resp / og1Resp / oldestSel / deqEntry。
//      条目的 deqSel = 任一端口选中;每个端口独立做三级 oldest mux(comp>simp>enq)。
//   2) **fuType_14(Fdiv)**:enq / deqEntry / fuType 输出现在保留 {11,12,14} 三位
//      (FaluFmac 只有 {11,12})。FuType 总位宽仍 35。
//   3) **status.deqPortIdx(1 bit)**:记录该条目将在哪个发射端口出队。
//      置位:被发射端口 1 选中(deqSelOH_1)时 → 1;否则发射端口 0 → 0。
//      用途:据此为该条目选择对应端口的 issueResp(见下)。
//   4) **og1Resp_1_bits_resp 动态 resp**:
//      - 端口 0(deqPortIdx=0):resp 硬编码——timer1=SUCCESS,其余 BLOCK(同 FaluFmac)。
//      - 端口 1(deqPortIdx=1):timer1 的 resp **取自 og1Resp_1_bits_resp**(Fdiv 多周期,
//        可能 BLOCK/UNCERTAIN/SUCCESS),timer0/2/3 = BLOCK。valid 同理按端口取 og*Resp_{0,1}.valid。
//
// 其余(srcStatus 无 ld_dep/RegCache/imm、og0Cancel 仅 [8]、is0Lat 仅 IQ 源 0、
//   enqDelay 1 级、UIntCompressor bit{8,10,12} 拓扑)与 FaluFmac 完全一致。
// =============================================================================
package iq_ffmacfdiv_pkg;

  // ---------------------------------------------------------------------------
  // 1. 变体维度参数(全部来自 golden EntriesFaluFmacFdiv.sv 端口)
  // ---------------------------------------------------------------------------
  ...
  localparam int NUM_DEQ       = 2;  // ★ 发射端口数(双端口:Falu/Fmac + Fdiv)★
  ...
  // ★ 与 FaluFmac 不同:多保留 fuType_14(Fdiv)★
  ...
  localparam int FU_FDIV  = 14; // 浮点除/开方(FaluFmacFdiv 新增)

  // FuType 紧凑存储:本变体只保留 {FU_FALU, FU_FMAC, FU_FDIV} 三位,其余 32 位恒 0
  // (golden 已按变体剪枝)。存储只留保留位,对外经 pack/unpack 复原 35 位 one-hot。
  localparam logic [FU_NUM-1:0] FU_TYPE_KEEP_MASK =
      (35'b1 << FU_FALU) | (35'b1 << FU_FMAC) | (35'b1 << FU_FDIV);
  localparam int FU_TYPE_KEEP_W = 3; // = $countones(FU_TYPE_KEEP_MASK)
  ...
  // ★ 本变体新增 deq_port_idx:该条目将在哪个发射端口出队(numDeq=2 才有意义)★
  typedef struct packed {
    ...
    logic                         deq_port_idx; // ★ 出队端口号(0=Falu/Fmac 口, 1=Fdiv 口)
  } status_t;
