#!/usr/bin/env python3
"""
EntriesVfmaVialuFixVimacVppuVfaluVfcvtVipuVsetrvfwvf 生成器
  (向量执行类、挂 FU 最多的双发射 IQ,从 gen_iq_vfmavialufixvfalu.py 派生)。

可读核手写在
  rtl/backend/{EntriesVfmaVialuFixVimacVppuVfaluVfcvtVipuVsetrvfwvf,
               IqEntryVfmaVialuFixVimacVppuVfaluVfcvtVipuVsetrvfwvf}.sv
  + iq_vfmavialufixvimacvppuvfaluvfcvtvipuvsetrvfwvf_pkg.sv
本脚本只生成「flat golden 端口 ↔ 可读核 struct 端口」的 wrapper 与双例化 tb。

★ 相对样板 VfmaVialuFixVfalu 的差异(全部增量)★
  维度完全相同(numEntries=16(2/2/12),numDeq=2,numRegSrc=5,numWakeupFromWB=16)。
  增量 A:fuType 由 3 位(19/24/25)扩为 8 位(18/19/20/21/24/25/27/30)。
  增量 B:payload 多带 rfWen / vlWen / selImm(透传字段)。
  增量 C:EntryBundle 顶层多带 imm(15 bit,透传字段)。
...
BK = XSSV / "rtl/backend"
UT = XSSV / "verif/ut/IssueQueueVfmaVialuFixVimacVppuVfaluVfcvtVipuVsetrvfwvf"
GEN_HDR = "// 自动生成:scripts/gen_iq_vfmavialufixvimacvppuvfaluvfcvtvipuvsetrvfwvf.py —— 勿手改\n"
PKG = "iq_vfmavialufixvimacvppuvfaluvfcvtvipuvsetrvfwvf_pkg"
...
MOD = "EntriesVfmaVialuFixVimacVppuVfaluVfcvtVipuVsetrvfwvf"
...
# ----------------------------------------------------------------------------
# ★ 增量 B:payload 新增 rfWen/vlWen/selImm ★
PAYLOAD_MAP = {
    "fuOpType": ".payload.fu_op_type",
    "rfWen": ".payload.rf_wen", "fpWen": ".payload.fp_wen",
    "vecWen": ".payload.vec_wen", "v0Wen": ".payload.v0_wen",
    "vlWen": ".payload.vl_wen",
    ...
    "selImm": ".payload.sel_imm",
    "pdest": ".payload.pdest",
}

# ★ 增量 A:8 位 fuType ↔ struct 字段 ★
FUTYPE_MAP = {
    "18": "vipu", "19": "vialuf", "20": "vppu", "21": "vimac",
    "24": "vfalu", "25": "vfma", "27": "vfcvt", "30": "vsetfwf",
}


def entry_field_expr(v, suffix):
    # ★ 增量 C:imm 是 EntryBundle 顶层字段(io_*_bits_imm,无 status_/payload_ 前缀)★
    if suffix == "imm":
        return f"{v}.imm"
    ...
    if m:
        return f"{v}.status.fu_type_" + FUTYPE_MAP[m.group(1)]
...
def emit_entries_wrapper():
    ...
    out.append(f"module {MOD} import {PKG}::*; (\n")
...
def emit_entries_tb():
    ...
    L = []
    L.append("// 自动生成:scripts/gen_iq_vfmavialufixvimacvppuvfaluvfcvtvipuvsetrvfwvf.py(entries tb)—— 勿手改\n")
