#!/usr/bin/env python3
"""生成 TLBuffer_20 (TL A/D 两通道窄总线 skid 缓冲) 的 wrapper / 变体 / UT / Makefile / 黑盒。

TLBuffer_20 是叶子级结构模块: A/D 两通道各例化一个 Queue2 黑盒。
  · UT 双例化: golden TLBuffer_20 vs 可读 TLBuffer_20_xs, 两侧共用真实 Queue2/ram 叶子,
    随机驱动全部输入、逐拍比对全部输出。
  · FM: ref=golden TLBuffer_20 (Queue 自动黑盒), impl=pkg+可读核+wrapper+黑盒声明。
"""
...
UT = ROOT / "verif" / "ut" / "TLBuffer_20"

TOP = "TLBuffer_20"
CORE = "xs_TLBuffer_20_core"
VARIANT = "TLBuffer_20_xs"
QUEUES = ["Queue2_TLBundleA_10", "Queue2_TLBundleD_11"]
RAMS = ["ram_2x115", "ram_2x78"]
...
def gen_wrapper(ports):
    return ("// 自动生成：scripts/gen_tlbuffer20.py —— 勿手改\n"
...
def gen_blackbox():
    out = ["// 自动生成：scripts/gen_tlbuffer20.py —— 勿手改",
...
def gen_tb(ports):
    ...
    L = [
        "// 自动生成：scripts/gen_tlbuffer20.py —— 勿手改",
        "// TLBuffer_20 双例化逐拍比对: golden TLBuffer_20 vs 可读 TLBuffer_20_xs。",
        "`timescale 1ns/1ps",
...
def gen_makefile():
    ...
    return f"""# 自动生成：scripts/gen_tlbuffer20.py —— 勿手改
MODULE = TLBuffer_20

RTL_DIR = ../../../rtl
GOLDEN_RTL = ../../../golden/chisel-rtl

RTL_SRCS = $(RTL_DIR)/memblock/tlbuffer20_pkg.sv \\
           $(RTL_DIR)/memblock/TLBuffer_20.sv

TB_SRCS = variants_xs.sv tb.sv

# UT 双例化: 两侧共用真实 Queue2 + ram 叶子。
GOLDEN_SRCS = $(GOLDEN_RTL)/TLBuffer_20.sv \\
              {golden_q} \\
              {golden_r}

WRAPPER_SRCS = $(RTL_DIR)/memblock/TLBuffer_20_wrapper.sv \\
               $(RTL_DIR)/memblock/tlbuffer20_queue_blackbox.sv
FM_VARIANTS = TLBuffer_20
# FM: ref = golden TLBuffer_20.sv (Queue 自动黑盒); impl = pkg + 可读核 + wrapper + 黑盒声明。
...
def main():
    ...
    (RTL / "tlbuffer20_queue_blackbox.sv").write_text(gen_blackbox())
