...
module tb;
  ...
  // IT: u_i = 全重写 _it 链顶层 DecodeStage_it (内部 DecodeUnit_it → xs_DecodeUnit_core_it
  //          → FPDecoder_it/UopInfoGen_it → xs_FPDecoder_core/xs_UopInfoGen_core 直连)。
  //     端口与 golden DecodeStage 逐字相同, 故与 u_g 共用同一套驱动/比对。
  DecodeStage_it u_i (
    .clock(clock),
    .reset(reset),
    .io_redirect(io_redirect),
...
`include "legal_instrs.svh"
  ...
  int n_iters = 250000;
  // +NCYCLES=<n>(IT 约定) 与 +ITERS=<n>(沿用 UT) 都可覆盖拍数; NCYCLES 优先。
  initial begin
    if ($value$plusargs("ITERS=%d",   n_iters)) ;
    if ($value$plusargs("NCYCLES=%d", n_iters)) ;
  end
  ...
  initial begin
    ...
    $display("[Decode IT] iters=%0d checks=%0d errors=%0d nonfunc_errors=0", n_iters, checks, errors);
