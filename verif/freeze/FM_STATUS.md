...
| **StorePipe** | **SUCCEEDED / SIGNOFF_PASS**(codex_0093 §2 vmucp integrator)。原因链: 本配置 golden 被 whole-chip firtool DCE 成 5 行空壳(0 输出/0 寄存器)→reference 读成 blackbox(FM-081),proof vacuous。**解法1(codex_0088 §3)** `reference_kind=canonical_derivative` 机械派生件 `G0-StorePipe-observable-v1`(冻结 SimTop.fir 7084336-7084662 抽取 + 锁定 firtool-1.62.1 standalone 重降,committed+hash 校验,无 env 覆盖)替换空壳。**解法2(codex_0092 §1 选项 B, 目标级语义面)** 两侧 StorePipe_surface 包装只再导出 36 defined 输出 + 23 输入, 14 个源 FIRRTL invalidate-only 叶(store_mask/store_data/amo_*/id/...)留内部悬空 net → 退出比较面(源派生 derive_surface.py + ledger hash 绑定; 非 dont_verify 非归0), 4 项负测 fail-closed。**解法3(codex_0093 §2 vmucp,本轮)** ①派生件 derive.sh 复算加 `--lowering-options=disallowLocalVariables`: 旧派生件把组合临时量(s1_tag_match_T_*/r_c_cat_T_*/_r_T/_GEN_5)当 automatic-local 留在时钟块内, FM 误 reify 成 **24 个 shadow DFF**(源/impl 均无, impl 核已用 G0 flags hoist 成组合 wire)——加 flag 后 0 automatic-local, 派生件寄存器集=11 真流水寄存器 与 impl 逐个对齐, IO 端口面字节不变。②余 6 个真 cone-dead = s2_paddr_reg[0..5](48 位 s2_paddr 低 6 位字节偏移, 块地址只用 [47:6], 两侧同源同宽)按名 set_user_match 双射(fm_pins.tcl, r:u_dut/... ↔ i:u_dut/u_core/..., 真等价非伪配)。③vmucp=true 令 FM 逐位实比这 6 点 → passing。**官方 gate native SUCCEEDED: 430 passing(266 Port + 164 DFF)/ 0 failing / 0(0) unmatched / 0 unread / 0 dont_verify / 0 relaxed。** |
