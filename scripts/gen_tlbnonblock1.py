#!/usr/bin/env python3
"""
TLBNonBlock_1 (store TLB, Width=2/nRespDups=1)：生成 golden 同名 wrapper(FM)+_xs(UT)+tb。

wrapper 例化 golden 黑盒 DelayN_9(sfence 2 拍)/DelayN_7(csr 2 拍)/TlbStorageWrapper_2
  (全相联存储+PLRU, NDUP=1)，与可读核 xs_TLBNonBlock_1_core(顶层控制)互连。
DelayN/TlbStorageWrapper_2 与 base 同法作黑盒(两侧同名 unresolved 对称匹配)。

设计意图: src/main/scala/xiangshan/cache/mmu/TLB.scala class TLBNonBlock(store TLB)。
可读核本体见 rtl/memblock/TLBNonBlock_1.sv。
"""
import re
...
def extract_inst_block(top, inst_decl):
    """从 golden top 里抓 `<Mod> <inst> ( ... );` 整块(含连接), 原样返回。"""
    ...
    started = False
    for ln in lines:
        if not started and inst_decl in ln:
            started = True
        if started:
            out.append(ln)
            depth += ln.count("(") - ln.count(")")
            if depth == 0 and started and ln.strip().endswith(");"):
                break
    return "\n".join(out)
...
BODY = r"""
  ...
  // ---- 请求端口打包(2 端口, store TLB 精简字段)----
  logic [1:0]        req_valid, req_checkfullva, req_hyperinst, req_robIdx_flag;
  logic [1:0][49:0]  req_vaddr;
  logic [1:0][63:0]  req_fullva;
  logic [1:0][2:0]   req_cmd;
  logic [1:0][7:0]   req_robIdx_value;
  assign req_valid       = {io_requestor_1_req_valid, io_requestor_0_req_valid};
  assign req_vaddr       = {io_requestor_1_req_bits_vaddr, io_requestor_0_req_bits_vaddr};
  assign req_fullva      = {io_requestor_1_req_bits_fullva, io_requestor_0_req_bits_fullva};
  assign req_checkfullva = {io_requestor_1_req_bits_checkfullva, io_requestor_0_req_bits_checkfullva};
  assign req_cmd         = {io_requestor_1_req_bits_cmd, io_requestor_0_req_bits_cmd};
  assign req_hyperinst   = {io_requestor_1_req_bits_hyperinst, io_requestor_0_req_bits_hyperinst};
  assign req_robIdx_flag = {io_requestor_1_req_bits_debug_robIdx_flag, io_requestor_0_req_bits_debug_robIdx_flag};
  assign req_robIdx_value= {io_requestor_1_req_bits_debug_robIdx_value, io_requestor_0_req_bits_debug_robIdx_value};

  // ---- s0 读口 + 存储读响应打包(NDUP=1)----
  wire [1:0]        e_rreq_valid; wire [1:0][37:0] e_rreq_vpn; wire [1:0][1:0] e_rreq_s2xlate;
  wire [1:0]        e_hit; wire [1:0][35:0] e_ppn0;
  wire [1:0][1:0]   e_pbmt, e_gpbmt, e_s2xlate;
  tlb_perm_t [1:0]  e_perm0; tlb_gperm_t [1:0] e_gperm0;

  wire e0_hit; wire [35:0] e0_ppn0; wire [1:0] e0_pbmt,e0_gpbmt,e0_s2x;
  ...
  wire e1_hit; wire [35:0] e1_ppn0; wire [1:0] e1_pbmt,e1_gpbmt,e1_s2x;
  ...
  assign e_s2xlate= {e1_s2x, e0_s2x};
  ...
  wire [1:0]        o_valid, o_miss, o_isForVS, o_vaNeedExt, o_isHyper;
  wire [1:0]        o_gpf_ld, o_gpf_st, o_pf_ld, o_pf_st, o_af_ld, o_af_st;
  wire [1:0][47:0]  o_paddr0; wire [1:0][63:0] o_gpaddr0, o_fullva; wire [1:0][1:0] o_pbmt0;
  ...
  xs_TLBNonBlock_1_core u_core (
    ...
    .io_req_robIdx_flag(req_robIdx_flag), .io_req_robIdx_value(req_robIdx_value),
    ...
    .io_entries_perm0(e_perm0), .io_entries_gperm0(e_gperm0), .io_entries_s2xlate(e_s2xlate),
    ...
    .io_resp_valid(o_valid), .io_resp_paddr0(o_paddr0),
  ...
  // ---- golden 黑盒 TlbStorageWrapper_2(存储+PLRU, NDUP=1)----
  ...
  // ---- 核 resp 拆回 golden 扁平输出(2 端口, 都完整)----
  ...
  assign io_requestor_1_resp_bits_excp_0_gpf_st = o_gpf_st[1];
  assign io_requestor_1_resp_bits_excp_0_pf_ld = o_pf_ld[1];
  assign io_requestor_1_resp_bits_excp_0_pf_st = o_pf_st[1];
  assign io_requestor_1_resp_bits_excp_0_af_ld = o_af_ld[1];
  assign io_requestor_1_resp_bits_excp_0_af_st = o_af_st[1];
...
def build_wrapper(modname):
    ...
    L = []
    L.append("// 自动生成：scripts/gen_tlbnonblock1.py —— 勿手改")
    L.append(f"module {modname} import xs_tlbnb1_pkg::*; (")
    L.append(",\n".join(decls))
    L.append(");")
    L.append(BODY)
    L.append("endmodule")
...
def build_tb(ps):
    ...
    T = ["// 自动生成：scripts/gen_tlbnonblock1.py —— 勿手改",
