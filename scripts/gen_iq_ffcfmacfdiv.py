#!/usr/bin/env python3
"""
IssueQueueFaluFcvtF2vFmacFdiv / EntriesFaluFcvtF2vFmacFdiv 生成器
(从 gen_iq_ffmacfdiv.py 派生)。

可读核手写在 rtl/backend/{EntriesFaluFcvtF2vFmacFdiv,IqEntryFaluFcvtF2vFmacFdiv}.sv
+ iq_ffcfmacfdiv_pkg.sv,本脚本只生成「flat golden 端口 ↔ 可读核 struct 端口」的
wrapper 与双例化 tb。

★ 与 FaluFmacFdiv 变体的差异(本变体加 Fcvt/F2v 两类 FU)★
  fuType 多保留两位:fuType_4(F2v)+ fuType_13(Fcvt)(原 {11,12,14} → {4,11,12,13,14})。
  payload 多两位写使能:vecWen + v0Wen(F2v 写向量/v0 寄存器)。
  其余(numDeq=2、deqPortIdx、og1Resp_1_bits_resp 动态 resp、唤醒机制)与 FaluFmacFdiv 一致。
...
BK = XSSV / "rtl/backend"
UT = XSSV / "verif/ut/IssueQueueFaluFcvtF2vFmacFdiv"
GEN_HDR = "// 自动生成:scripts/gen_iq_ffcfmacfdiv.py —— 勿手改\n"

# ---- 维度参数(与 iq_ffcfmacfdiv_pkg 一致)----
...
MOD = "EntriesFaluFcvtF2vFmacFdiv"
...
# (FaluFcvtF2vFmacFdiv:fuType_{4,13} 同 [n] 通配;payload 多 vecWen/v0Wen)
# ----------------------------------------------------------------------------
...
def emit_entries_wrapper():
    ...
    out = [GEN_HDR]
    out.append("// %s golden 同名扁平 wrapper —— 例化可读核 xs_%s_core。\n" % (MOD, MOD))
    out.append("module %s import iq_ffcfmacfdiv_pkg::*; (\n" % MOD)
...
def emit_entries_tb():
    ...
    L = []
    L.append("// 自动生成:scripts/gen_iq_ffcfmacfdiv.py(entries tb)—— 勿手改\n")
...
def emit_entries_xs_variant():
    ...
    txt = txt.replace("module %s " % MOD, "module %s_xs " % MOD)
