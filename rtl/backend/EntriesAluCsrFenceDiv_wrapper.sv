// 自动生成:scripts/gen_iq_acfd.py —— 勿手改
// EntriesAluCsrFenceDiv golden 同名扁平 wrapper —— 例化可读核 xs_EntriesAluCsrFenceDiv_core。
module EntriesAluCsrFenceDiv import iq_acfd_pkg::*; (
  input  clock,
  input  reset,
  ...
  always_comb begin
    ...
    // 外部只送 fuType_{5,6,8,9} 四位,打包进紧凑存储(见 pack_fu_type)
    ...
    c_og1resp_resp_p1 = resp_type_e'(io_og1Resp_1_bits_resp);
  ...
  //   先解包到中间线网,再按位索引(FM 前端不支持对函数返回值直接下标)。
  ...
  assign io_deqEntry_1_bits_payload_flushPipe = c_deq_entry[1].payload.flush_pipe;
  ...
  xs_EntriesAluCsrFenceDiv_core u_core (
