...
valid 判定（`Sbuffer.sv:547-549`）：

```
out_s0_valid = missqReplayHasTimeOut
             | ( isDcacheReqCandidate(stateVec[evictionIdx]) & (needDrain | cohHasTimeOut | need_replace) )
```

> ⚠ **重发超时路径绕过 `isDcacheReqCandidate` 检查**：`missqReplayHasTimeOut` **直接** OR 进 valid，
> **不过** `isDcacheReqCandidate` 门。原因——被 replay 的 entry 在上一次逐出（`out_s0` fire）时已置
> `state_inflight=1`（`:668`），而 `isDcacheReqCandidate = valid & !inflight & !w_sameblock_inflight`
> 对 inflight entry 恒为假；若重发也要过该门，这条 entry 就**永远重发不出去**。故只有 drain / cohTimeOut
> / replace 三类触发才要求 entry 是 DCache 写候选，重发超时特批放行。（entry 生命周期：逐出→
> `inflight=1`(`:668`)；`replay_resp`→`w_timeout=1`(`:685`)；命中回应 `hit_resp`→清 `valid/inflight`(`:674-675`)。）

`out_s0` fire 时把 entry 置 `state_inflight`、清 `w_timeout`；`out_s1` 流水寄存器锁存 idx/ptag/vtag，组
DCache 写请求：整行 `data(512b)` / `mask(64b)`、`addr = ptag<<6`、`id = evictionIdx`。
...
- **f1**：打一拍后用 `forward.paddr` 复核（ptag CAM）。`Mux1H` 选中命中 entry 的 mask/data：
  **active(valid) 优先于 inflight**。逐字节输出 `forwardMask = inflMask|validMask`、
  `forwardData = validMask[j] ? validData[j] : inflData[j]`。`matchInvalid` 的判据是
  **vtag 命中与 ptag 命中不一致（双向异或）**：`(vtag_match != ptag_match) & (active | inflight)`
  （`Sbuffer.sv:845`）——**既包括「vtag 命中而 ptag 失配」，也包括「ptag 命中而 vtag 失配」**，
  两方向都说明 vaddr↔paddr 别名冲突，触发 `forward_need_uarch_drain`（抽干 buffer 重做）。
...
flowchart LR
    ...
    chk -->|"vtag命中 ⊕ ptag命中"| minv["matchInvalid + drain"]
```
