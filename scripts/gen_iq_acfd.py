...
「flat golden 端口 ↔ 可读核 struct 端口」的 wrapper 与双例化 tb:
  ...
  Entries:
    EntriesAluCsrFenceDiv_wrapper.sv  —— golden 同名扁平 wrapper(例化可读核),FM/UT 用。
    verif/ut/IssueQueueAluCsrFenceDiv/entries_tb.sv —— 双例化(u_g golden vs u_i wrapper)
        逐拍比对全部输出。单条目两侧:u_g 用 golden EnqEntry_6/OthersEntry_66/72;
        u_i 用可读核 xs_iq_entry_acfd。EnqPolicy/UIntCompressor 两侧共享 golden 黑盒。
...
BK = XSSV / "rtl/backend"
UT = XSSV / "verif/ut/IssueQueueAluCsrFenceDiv"
GEN_HDR = "// 自动生成:scripts/gen_iq_acfd.py —— 勿手改\n"

# ---- 维度参数(与 iq_acfd_pkg 一致)----
...
# ----------------------------------------------------------------------------
# entry_t 字段 ↔ golden flat 字段后缀 的映射
# golden 后缀(去掉 "status_"/"payload_"/"imm" 前缀的层级)→ core struct 路径
# 返回 core 端 SV 表达式片段(相对某 entry_t 变量 v)
# ----------------------------------------------------------------------------
def entry_field_expr(v, suffix):
    """suffix 例:'status_srcStatus_0_psrc' / 'imm' / 'payload_pdest'。"""
    if suffix == "imm":
        return f"{v}.imm"
    ...
    if m:
        ...
        fmap = {
            ...
            "rfWen": ".payload.rf_wen", "flushPipe": ".payload.flush_pipe",
...
# ----------------------------------------------------------------------------
# 生成每个 flat 输入端口 → core 输入信号的「打包」赋值,以及 core 输出 → flat 输出。
# core 输入用中间 struct 变量(enq_bits[NUM_ENQ], wk_wb[], wk_iq[], ...)。
# ----------------------------------------------------------------------------
def gen_entries_wrapper(ports):
    pack = []   # always_comb 内的赋值(输入打包)
    unpack = [] # 输出解包(assign)
    for d, w, n in ports:
        body = n[len("io_"):]
        # ---- enq ----
        ...
        if body == "og1Resp_1_bits_resp":
            pack.append(f"    c_og1resp_resp_p1 = resp_type_e'({n});")
            continue
        # ---- deq sel ----
        m = re.match(r"deqReady_(\d)$", body) or re.match(r"deqReady$", body)
        ...
        # deqReady has no [d] (width NUM_DEQ vector?) handle generic
        if body.startswith("deqReady"):
            mm = re.match(r"deqReady_(\d)$", body)
            if mm:
                pack.append(f"    c_deq_ready[{mm.group(1)}] = {n};"); continue
...
ENT_CORE_PORTS = """\
    ...
    .og1resp_resp_p1(c_og1resp_resp_p1),
...
def emit_entries_wrapper():
    ports = parse_ports("EntriesAluCsrFenceDiv", "EntriesAluCsrFenceDiv.sv")
    pack, unpack = gen_entries_wrapper(ports)
    out = [GEN_HDR]
    out.append("// EntriesAluCsrFenceDiv golden 同名扁平 wrapper —— 例化可读核 xs_EntriesAluCsrFenceDiv_core。\n")
    out.append("module EntriesAluCsrFenceDiv import iq_acfd_pkg::*; (\n")
    ...
    out.append("    c_og1cancel = '0; // Entries 顶层未引 og1Cancel(条目逻辑不消费),置 0\n")
    out.append("    c_og0cancel = '0;\n")
    # golden 把 enq.bits 中本变体不消费的字段(exuSources / srcLoadDependency(status侧) /
    # issued / issueTimer / deqPortIdx / blocked / firstIssue)裁成常量 0,故未引出为端口。
    # 这里把整组 enq_bits 先清 0,再按存在的端口逐字段覆盖,避免未连字段悬空成 X。
    out.append("    c_enq_bits = '0;\n")
    # 唤醒源 4/5/6 无 loadDependency 端口(golden 裁为 0);ldCancel ld1 全局未接。先整组清 0。
    out.append("    c_wk_iq = '0; c_wk_iq_d = '0; c_wk_wb = '0; c_wk_wb_d = '0; c_ldcancel = '0;\n")
    out.extend(p + "\n" for p in pack)
    out.append("  end\n")
    out.extend(u + "\n" for u in unpack)
    out.append("  xs_EntriesAluCsrFenceDiv_core u_core (\n")
    out.append(ENT_CORE_PORTS)
    out.append("  );\nendmodule\n")
    (BK / "EntriesAluCsrFenceDiv_wrapper.sv").write_text("".join(out))
    print("wrote EntriesAluCsrFenceDiv_wrapper.sv  (ports=%d pack=%d unpack=%d)"
          % (len(ports), len(pack), len(unpack)))


# ----------------------------------------------------------------------------
# 双例化 tb 生成:对 EntriesAluCsrFenceDiv 的全部输入随机激励,逐拍比对全部输出。
# u_g = golden EntriesAluCsrFenceDiv(例化 golden 单条目);u_i = 同名 wrapper(可读核)。
# 为避免重名冲突,u_i 用 wrapper 同名模块 —— 改在不同 scope 中用 `bind`? 否:
# golden 与 wrapper 同名 EntriesAluCsrFenceDiv 不能共存。故 wrapper 在 tb 编译时
# 重命名为 EntriesAluCsrFenceDiv_xs(由脚本另出 variants 文件)。
# ----------------------------------------------------------------------------
def emit_entries_tb():
    ports = parse_ports("EntriesAluCsrFenceDiv", "EntriesAluCsrFenceDiv.sv")
    ...
    L = []
    L.append("// 自动生成:scripts/gen_iq_acfd.py(entries tb)—— 勿手改\n")
    ...
    def inst(mod, pfx):
        ...
        s.append("\n  );\n")
        return "".join(s)
    L.append(inst("EntriesAluCsrFenceDiv", "g"))
    L.append(inst("EntriesAluCsrFenceDiv_xs", "i"))
    # random stim: drive all inputs each cycle
    L.append("  task drive_rand();\n")
    for w, n in ins:
        # keep widths; randomize
        L.append(f"    {n} = $urandom;\n")
    # bias valids/handshakes lower so queue fills/drains realistically
...
def emit_entries_xs_variant():
    """wrapper 重命名为 EntriesAluCsrFenceDiv_xs 供 tb 与 golden 共存。"""
    txt = (BK / "EntriesAluCsrFenceDiv_wrapper.sv").read_text()
    txt = txt.replace("module EntriesAluCsrFenceDiv ", "module EntriesAluCsrFenceDiv_xs ")
...
# ============================================================================
# 单条目隔离 UT:golden EnqEntry_6 / OthersEntry_66 / OthersEntry_72 vs 可读核
# xs_iq_entry_acfd。golden 的 commonIn/commonOut/enqDelayIn 扁平端口直接映射到
# 可读核同义端口。enqDelay 输入直接驱动(数组里才需推导),便于隔离收敛。
# ============================================================================
ENTRY_VARIANTS = {
    # name        : (golden_file, IS_ENQ, IS_COMP, has_enqdelay)
    "EnqEntry_6":   ("EnqEntry_6.sv",   1, 1, True),
    "OthersEntry_66":("OthersEntry_66.sv",0, 0, False),
    "OthersEntry_72":("OthersEntry_72.sv",0, 1, False),
}


def _entry_in_map(body):
    """golden 单条目 input(去 io_ 前缀) → 可读核输入信号表达式。返回 (core_sig_assign)。
    用于 tb 把随机激励同时喂 golden 与可读核 wrapper。这里直接生成「可读核 wrapper 端口
    赋值」。返回 None 表示该端口可读核不消费(tie/忽略)。"""
    # flush
    fmap = {"commonIn_flush_valid":"flush_valid","commonIn_flush_bits_robIdx_flag":"flush_rob_flag",
            "commonIn_flush_bits_robIdx_value":"flush_rob_value","commonIn_flush_bits_level":"flush_level",
            "commonIn_enq_valid":"enq_valid","commonIn_deqSel":"deq_sel",
            "commonIn_deqPortIdxWrite":"deq_port_write","commonIn_transSel":"trans_sel",
            "commonIn_issueResp_valid":"issue_resp.valid"}
    if body in fmap:
        return ("scalar", fmap[body])
    if body == "commonIn_issueResp_bits_resp":
        return ("resp", "issue_resp.resp")
    return None
