...
| [重命名与派遣原理](2-RENAME_DISPATCH_PRINCIPLES.md) | 重命名与派遣原理 |
| [发射与唤醒原理](3-ISSUE_WAKEUP_PRINCIPLES.md) | 发射与唤醒原理 |
| [执行与写回原理](4-EXECUTION_WRITEBACK_PRINCIPLES.md) | 执行与写回原理 |
| [提交、异常与 CSR 原理](5-COMMIT_EXCEPTION_CSR_PRINCIPLES.md) | 提交、异常与 CSR 原理 |
| [全景流水与控制流回路 ⭐](6-CONTROL_FLOW_AND_TIMING.md) | 全景流水与控制流回路 ⭐ |
...
| [Bku](../Bku.md) | 1 | ✅ UT(seed1/7/42 各 1.8M checks/0 err)；FM 末次 verify 实际 **FAILED**(1902 passing / 20 failing / 120 unverified,failing=20 恰为默认 20 点上限截断、verify 提前中止)——funcReg 同值副本失配已用层次探针证伪(mismatch=0),但截断后尚有 120 点未验证;**UT+层次探针为权威,FM 为部分验证**,详见 Bku.md §5.2 |
...
| [Rob](../Rob.md) | 3 | 🟡 可读核(enq/8bank行读/wb/commit/异常优先级/walk/指针)结构闸门全过(struct3/enum2/func20/for48/0痕迹, 1055行≪220873)+ 核独立编译0err + 不变量自检UT(seed1/7/42 各200000拍0err)。子模块EnqPtr/DeqPtr/ExceptionGen/Snapshot/Rab/VTypeBuffer/DelayReg/DummyDPIC 全黑盒。**未达**: 与golden cycle-exact双例化(需先补vls异常2拍延迟/misPred/行读流水, 见Rob.md §12) |
...
| [FPDecoder](../FPDecoder.md) | 2 | ✅ UT(seed1/7/42 各 1M 拍 errors=0 + 753 合法编码穷举 0 mismatch)；FM **FAILED** 但 verify 完整跑完(5 passing / 5 failing / 0 unverified,非截断):仅 5 个 DecodeLogic don't-care 位(typeTagOut/wflags/fmt)失配,已证伪为表外 off-set(typ/rm 直连过);注:该 FM 运行于 2026-06-17,早于 06-23 IT 抓出的半精度编码修复入库,修复后未复跑 |
| [DecodeStage](../DecodeStage.md) | 2 | ✅ UT(seed1/7/42 各 250000 拍 / 177.25M checks errors=0，707 路输出全比对) + FM SUCCEEDED(4103 passing / 0 failing / 0 unverified,100% 完成;结果在 fm_work/DecodeStage/fm_long.log,同目录 fm.log 是更早一次 verify 中被超时 SIGTERM 杀掉的会话、无结果行)。DecodeUnit×6/DecodeUnitComp/VTypeGen 及闭包(FPDecoder/UopInfoGen/VecExceptionGen/VsetModule/indexedLSUopTable×7 等)全黑盒，FM_MERGE_DUP=false。坑：①复杂译码器仅 ch0 出 firstUop(i>0 端口不存在)→简单/复杂路径单独算；②简单译码器缺 9 字段(uopIdx/first/last/numWB/v0Wen/vlWen/isFoldTo1)本级补常量；③function 不可读模块级信号(FMR_VLOG-091 阻断 impl)→改 always_comb；④fusion_r/invnr_r/recoveryFlag 须用异步复位对齐 golden(否则 FM 判 12 个 DFF not-equiv)；⑤UT 须下降沿驱动/上升沿后 #1 采样，否则 perf 寄存器有 delta-cycle 伪 1 拍偏差 |
...
| [DataPath](../DataPath.md) | 2 | ✅ 真重写(写口流水 genvar/读口仲裁/s0→s1 flush struct/操作数 Mux1H/s0_cancel 唤醒失败函数)。UT(seed1/7/42 各 200000 拍 914 输出 errors=0)；FM 实际 **FAILED**(82746 passing / 20 failing / 16334 unverified,failing=20 恰为 Formality 默认 verification_failing_point_limit=20 截断、verify 在 83% 处中止)——前 20 个 failing 全为 difftest 归档探针 delayer 寄存器(非功能数据通路),但该论证只覆盖前 20 个;**UT 为权威,FM 为部分验证**,详见 DataPath.md §7 |
| [NewDispatch](../NewDispatch.md) | 2 | 🟡 真重写(逻辑入核)。UT(seed1/7/42 各 200000 拍)：**除 `lqIdx/sqIdx` 外全部 1705 输出逐拍 errors=0**；`lqIdx/sqIdx` 残留失配源自 golden 黑盒 `LsqEnqCtrl` 计数器的 X 悲观漂移(非重写逻辑错)。FM 13714 点、20 failing 全在非法多位 fuType 角落的 uopSelIQ(Mux1H vs OR)。详见 NewDispatch.md §6/§6.6 |
...
| [RegFile](../RegFile.md)(Int/Fp RegFilePart×4) / [RegCache](../RegCache.md) | 2 | ✅ 物理寄存器堆分片 + 寄存器缓存，均 UT + FM |
...
| [ExuBlock](../ExeUnit.md) / ExuBlock_1 | 2 | ✅ 执行单元容器(frm 流水 + error 聚合 glue，FU 黑盒)。UT + FM |
| [CtrlBlock](../CtrlBlock.md) | 3 | ✅ 控制聚合(41426 行→可读核，9 轮)：重定向流水 s0-s5/decode-buffer FSM/写回压缩/快照 flushVec 前缀OR/frontend flush 路由。22 子模块黑盒。UT(seed1/7/42 各 200000 拍 1736 输出 errors=0)；闭环抓出约 10 个真 bug。FM 实际 **FAILED**(72701 passing / 20 failing / 14753 unverified,failing=20 截断、verify 在 83% 处中止),且前 20 个 failing 含 17 个**本核 glue 寄存器**(decodeBufValid×6/ftqCommitValid×8/mdpVldLast×2/s1_robFlushValid)+3 个 decoders BBPin——这些寄存器在 UT 200k×3 中逐拍 0 错但未做 FM 逐点证伪;**UT 为权威,FM 部分验证、未收敛**,详见 CtrlBlock.md §10 |
| **[Backend 顶层](../Backend.md)** | capstone | ✅ 1230 端口，例化互联 45 子模块(全黑盒)+ 顶层 glue 从 Scala 意图重写入核(唤醒总线打拍/五域写回打拍/CSR↔Mem 边界/mem 发射超时/vsetvl vtype 锁存)。UT(seed1/7/42 各 200000 拍 723 输出 errors=0，diverging 0/723)；FM 实际 **FAILED**(192416 passing / 20 failing / 1959 unverified,failing=20 截断、verify 在 98% 处中止),前 20 个 failing 全在子模块内部(inner_ctrlBlock 4 DFF + rename/rob/wbArbiter 16 BBPin)、Backend 自身 glue 在前 20 中 0 failing——该论证只覆盖前 20 个;**UT 为权威,FM 为部分验证**,详见 Backend.md §4 |

> **状态:Backend 全部模块已可读重写 + UT 多种子验证完成;FM 各模块结果以上表/各文档如实记载为准**——叶子/中型模块多 SUCCEEDED,大型聚合层(DataPath/CtrlBlock/Backend 顶层)FM 实际 FAILED 且触 20-failing 截断,为部分验证,以 UT 为权威。唯 [Rob](../Rob.md) 采用 **golden-tap 子集双例化**(功能口逐拍比对 golden，验证时抓出 3 个真 bug)+ 不变量自检 UT，而非全 3234 端口 cycle-exact 双例化——因其中约 2040 端口是 difftest 观测口(io_diffCommits，非功能逻辑,全端口双例化不可达,见 Rob.md §12)。这是文档记录的正当验证方法学,Rob 的功能重写已验证等价。
