#!/usr/bin/env python3
"""
EntriesVlduVstuVseglduVsegstu 生成器(向量/段 load/store IQ,从 gen_iq_vlduvstu.py 派生)。

可读核手写在 rtl/backend/{EntriesVlduVstuSeg,IqEntryVlduVstuSeg}.sv +
iq_vlduvstuseg_pkg.sv,本脚本只生成「flat golden 端口 ↔ 可读核 struct 端口」的
wrapper 与双例化 tb。

★ 与 VlduVstu 样板的唯一差异:fuType one-hot 多 2 位 VSEGLDU=33 / VSEGSTU=34
  (段 load/store),在条目里随 status 透传,行为与 VLDU/VSTU 逐字一致。★

继承自 VlduVstu(本变体不变):
  numEntries 16(2/2/12);条目带 blocked + vecMem(sqIdx/lqIdx/numLsElem);
  payload 为向量访存全集(vpu.vmask/nf/veew/isVleff、lastUop、lqIdx、sqIdx);
  canIssue 多 & ~blocked;mem 响应三路按 {sqIdx,lqIdx} 匹配在 issueTimer 饱和窗口折算。
...
BK = XSSV / "rtl/backend"
UT = XSSV / "verif/ut/IssueQueueVlduVstuVseglduVsegstu"
GEN_HDR = "// 自动生成:scripts/gen_iq_vlduvstuseg.py —— 勿手改\n"
GOLDEN_MOD = "EntriesVlduVstuVseglduVsegstu"  # golden 顶层 Entries 模块名
CORE_MOD = "xs_EntriesVlduVstuSeg_core"       # 可读核阵列模块名
PKG = "iq_vlduvstuseg_pkg"

# ---- 维度参数(与 iq_vlduvstuseg_pkg 一致)----
...
def entry_field_expr(v, suffix):
    ...
    if m:
        fu_map = {"31": "vldu", "32": "vstu", "33": "vsegldu", "34": "vsegstu"}
        return f"{v}.status.fu_type_" + fu_map[m.group(1)]
...
def emit_entries_wrapper():
    ports = parse_ports(GOLDEN_MOD, GOLDEN_MOD + ".sv")
    pack, unpack = gen_entries_wrapper(ports)
    out = [GEN_HDR]
    out.append(f"// {GOLDEN_MOD} golden 同名扁平 wrapper —— 例化可读核 {CORE_MOD}。\n")
    out.append(f"module {GOLDEN_MOD} import {PKG}::*; (\n")
    ...
    out.append("  end\n")
    out.extend(u + "\n" for u in unpack)
    out.append(f"  {CORE_MOD} u_core (\n")
    out.append(ENT_CORE_PORTS)
    out.append("  );\nendmodule\n")
    (BK / (GOLDEN_MOD + "_wrapper.sv")).write_text("".join(out))
    print("wrote %s_wrapper.sv  (ports=%d pack=%d unpack=%d)"
          % (GOLDEN_MOD, len(ports), len(pack), len(unpack)))
...
def emit_entries_tb():
    ...
    L = []
    L.append("// 自动生成:scripts/gen_iq_vlduvstuseg.py(entries tb)—— 勿手改\n")
    ...
    def inst(mod, pfx):
        ...
        s.append("\n  );\n")
        return "".join(s)
    L.append(inst(GOLDEN_MOD, "g"))
    L.append(inst(GOLDEN_MOD + "_xs", "i"))
...
def emit_entries_xs_variant():
    txt = (BK / (GOLDEN_MOD + "_wrapper.sv")).read_text()
    txt = txt.replace(f"module {GOLDEN_MOD} ", f"module {GOLDEN_MOD}_xs ")
