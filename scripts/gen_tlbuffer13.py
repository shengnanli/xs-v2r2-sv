#!/usr/bin/env python3
"""生成 TLBuffer_13 (4×bank TL-C 五通道 skid 缓冲) 的 wrapper / 变体 / UT / Makefile / 黑盒。

TLBuffer_13 = 4 个相同 bank, 每 bank 5 通道各一个 Queue2, 共 20 个 Queue 实例 / 374 端口。
可读核拆为 tlbuffer13_pkg + xs_tlbuffer13_bank (单 bank) + xs_TLBuffer_13_core (外层 genvar banking)。
  · UT 双例化: golden TLBuffer_13 vs 可读 TLBuffer_13_xs, 两侧共用真实 Queue2/ram 叶子,
    随机驱动全部输入、逐拍比对全部输出。
  · FM: ref=golden TLBuffer_13 (Queue 自动黑盒), impl=pkg+bank+核+wrapper+黑盒声明。
"""
...
UT = ROOT / "verif" / "ut" / "TLBuffer_13"

TOP = "TLBuffer_13"
CORE = "xs_TLBuffer_13_core"
VARIANT = "TLBuffer_13_xs"
QUEUES = ["Queue2_TLBundleA_19", "Queue2_TLBundleB_1", "Queue2_TLBundleC_1",
          "Queue2_TLBundleD_20", "Queue2_TLBundleE_1"]
RAMS = ["ram_2x119", "ram_2x352", "ram_2x321", "ram_2x283", "ram_sink_2x8"]
...
def gen_wrapper(ports):
    return ("// 自动生成：scripts/gen_tlbuffer13.py —— 勿手改\n"
...
def gen_blackbox():
    out = ["// 自动生成：scripts/gen_tlbuffer13.py —— 勿手改",
...
def gen_makefile():
    ...
    return f"""# 自动生成：scripts/gen_tlbuffer13.py —— 勿手改
MODULE = TLBuffer_13

RTL_DIR = ../../../rtl
GOLDEN_RTL = ../../../golden/chisel-rtl

RTL_SRCS = $(RTL_DIR)/memblock/tlbuffer13_pkg.sv \\
           $(RTL_DIR)/memblock/xs_tlbuffer13_bank.sv \\
           $(RTL_DIR)/memblock/TLBuffer_13.sv

TB_SRCS = variants_xs.sv tb.sv

# UT 双例化: 两侧共用真实 Queue2 + ram 叶子。
GOLDEN_SRCS = $(GOLDEN_RTL)/TLBuffer_13.sv \\
              {golden_q} \\
              {golden_r}

WRAPPER_SRCS = $(RTL_DIR)/memblock/TLBuffer_13_wrapper.sv \\
               $(RTL_DIR)/memblock/tlbuffer13_queue_blackbox.sv
FM_VARIANTS = TLBuffer_13
# FM: ref = golden TLBuffer_13.sv (Queue 自动黑盒); impl = pkg + bank + 可读核 + wrapper + 黑盒声明。
...
def main():
    ...
    (RTL / "tlbuffer13_queue_blackbox.sv").write_text(gen_blackbox())
