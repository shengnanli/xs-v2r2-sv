// =============================================================================
// xs_EntriesFaluFmacFdiv_core —— 香山 V2R2(昆明湖) 发射队列条目阵列 可读重写
//                                (FaluFmacFdiv 变体, hasCompAndSimp 路)
// -----------------------------------------------------------------------------
// 设计源:src/main/scala/xiangshan/backend/issue/Entries.scala
// golden 对照:EntriesFaluFmacFdiv.sv(例化 2×EnqEntry_10 + 2×OthersEntry_104(simp)
//             + 14×OthersEntry_106(comp) + 2×EnqPolicy_8(simp/comp 转移策略))
//
// ★ 本模块 = 条目阵列 + 转移策略 + 年龄最老选择(每发射端口一套)★
// 整体框架与 FaluFmac 样板(xs_EntriesFaluFmac_core)同构,差异仅在「双发射端口」:
//
...
//        - 端口 0:resp 硬编码 timer1=SUCCESS(同 FaluFmac);valid 取 og{0,1}Resp_0。
...
//   5) fuType 现含 Fdiv 位(由单条目核携带,本层只汇聚)。
//
// 转移策略(EnqPolicy / finalSimp/CompTransSel / simp→comp 提级)与 FaluFmac 逐字相同。
// X 铁律:oldest/Mux1H 用「sel[i] ? entry[i] : 0」累加(OR),sel 为 0 时不引 X。
// =============================================================================
module xs_EntriesFaluFmacFdiv_core import iq_ffmacfdiv_pkg::*; (
  input  logic clock,
  input  logic reset,
  ...
  generate
    for (e = 0; e < NUM_ENTRIES; e++) begin : g_entry
      ...
      xs_iq_entry_ffmacfdiv #(.IS_ENQ(IS_ENQ_E), .IS_COMP(IS_COMP_E)) u_ety (
        .clock          (clock),
        .reset          (reset),
