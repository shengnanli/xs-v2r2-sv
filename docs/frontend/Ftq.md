...
| # | 通道 | 关键信号 | 说明 |
|---|------|---------|------|
| ① | **enq from BPU** | `bpu_in_fire`/`bpu_in_ptr`/`bpu_target`/`cfi_*` | s3>s2>s1 三级选择（`selectedResp`）；s2/s3 可覆盖更早预测并令 `bpuPtr` 回退；据 full_pred 算 cfiIndex（取首个 taken 槽的 offset，存 `cfiIndex_valid[]`/`cfiIndex_bits[]`）与 target（按 taken 选目标） |
...
`cfiIndex_valid[]`/`cfiIndex_bits[]`（RTL 实为两个并列数组，非单个 `cfiIndex_vec`）、`mispred_block`、`entry_fetch_status`、`entry_hit_status`、`pred_stage`、
