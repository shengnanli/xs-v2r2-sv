...
### 6.3 FM
golden 顶层（纯叶子）vs 可读同名 wrapper（→ 可读核）。结果：**1055 passing / 20 inconclusive**，
inconclusive 全部为 `u_core/req_r_reg[fullva]`，根因是寄存器边界命名差异：
...
`req_r[fullva]` 比对点判为 inconclusive。**已用 tb 内部层次探针证伪**：在 200000 拍内逐拍
直接比对 `u_g.req_fullva/req_gpaddr/req_uop_robIdx_value/req_uop_uopIdx` 与可读核
`u_core.req_r.*`，三种子 **probe_mm = 0**（数值恒等）。叠加 UT 5 个输出 0 错，功能等价确证。
