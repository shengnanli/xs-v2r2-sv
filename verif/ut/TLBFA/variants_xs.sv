...
module TLBFA_xs import xs_tlbfa_pkg::*; (
  input clock,
  input reset,
  ...
  logic          resp_valid[3];
  logic          resp_hit  [3];
  logic [35:0]   resp_ppn    [3][1];
  logic [1:0]    resp_pbmt   [3][1];
  logic [1:0]    resp_g_pbmt [3][1];
  tlb_perm_t     resp_perm   [3][1];
  tlb_gperm_t    resp_g_perm [3][1];
  logic [1:0]    resp_s2xlate[3][1];
  tlb_refill_t   wdata;
  logic          acc_valid [3];
  logic [5:0] acc_bits  [3];
  always_comb begin
...
// TLBFA golden 同名 / xs 变体扁平包装：扁平端口 <-> 可读核 struct/数组端口。
// 输出端口集合按 golden 实测裁剪结果生成（firtool 死代码消除非均匀）。
module TLBFA_1_xs import xs_tlbfa_pkg::*; (
  input clock,
  input reset,
  input io_sfence_valid,
  input io_sfence_bits_rs1,
