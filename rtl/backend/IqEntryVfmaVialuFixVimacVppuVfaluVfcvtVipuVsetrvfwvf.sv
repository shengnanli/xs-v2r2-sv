...
//   —— 香山 V2R2(昆明湖) 发射队列「单条目」可读重写
//      (VfmaVialuFixVimacVppuVfaluVfcvtVipuVsetrvfwvf 变体)
// -----------------------------------------------------------------------------
// 设计源:src/main/scala/xiangshan/backend/issue/{EntryBundles,EnqEntry,OthersEntry}.scala
// golden 对照:EnqEntry_14.sv(入队条目) / OthersEntry_136.sv(简单条目, 转移源)
//             / OthersEntry_138.sv(复杂条目, 终端)
//
// ★ 本条目 = 样板 VfmaVialuFixVfalu 单条目 + 三处「纯透传」增量(见 pkg 注释)★
//   增量 A:fuType 由 3 位扩为 8 位(vipu/vialuF/vppu/vimac/vfalu/vfma/vfcvt/vsetfwf)。
//   增量 B:payload 多带 rf_wen / vl_wen / sel_imm(仅寄存透传)。
//   增量 C:entry_t 顶层多带 imm(仅寄存透传)。
// 其余(唤醒分组 / ignoreOldVd / issued / issueTimer / deqPortIdx / 转移 / 入队延迟)
// 与样板逐字相同——增量字段都不参与唤醒/选择判定。
//
// 三种条目变体由参数选择:
...
// =============================================================================
module xs_iq_entry_vfmavialufixvimacvppuvfaluvfcvtvipuvsetrvfwvf
  ...
  // ===========================================================================
  // 3. ★ ignoreOldVd(向量专属,仅 src2=旧 vd)★(与样板逐字相同)
  ...
  always_comb begin
    ...
    // fuType 只保留本变体功能位(IQFuType.readFuType);8 位逐位透传。
    entry_update.status.fu_type_vipu    = entry_reg.status.fu_type_vipu;
    entry_update.status.fu_type_vialuf  = entry_reg.status.fu_type_vialuf;
    entry_update.status.fu_type_vppu    = entry_reg.status.fu_type_vppu;
    entry_update.status.fu_type_vimac   = entry_reg.status.fu_type_vimac;
    ...
    entry_update.status.fu_type_vfcvt   = entry_reg.status.fu_type_vfcvt;
    entry_update.status.fu_type_vsetfwf = entry_reg.status.fu_type_vsetfwf;
  ...
  // fuType 输出(仅本变体 8 个功能位)
  always_comb begin
    o_fu_type = '0;
    o_fu_type[FU_VIPU]    = entry_reg.status.fu_type_vipu;
    o_fu_type[FU_VIALUF]  = entry_reg.status.fu_type_vialuf;
    o_fu_type[FU_VPPU]    = entry_reg.status.fu_type_vppu;
    o_fu_type[FU_VIMAC]   = entry_reg.status.fu_type_vimac;
    o_fu_type[FU_VFALU]   = entry_reg.status.fu_type_vfalu;
    o_fu_type[FU_VFMA]    = entry_reg.status.fu_type_vfma;
    o_fu_type[FU_VFCVT]   = entry_reg.status.fu_type_vfcvt;
    o_fu_type[FU_VSETFWF] = entry_reg.status.fu_type_vsetfwf;
  end
