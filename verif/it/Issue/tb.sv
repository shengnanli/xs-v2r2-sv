// =============================================================================
// Issue(Int Scheduler)簇 IT testbench —— 基于 verif/ut/Scheduler/sched_tb.sv 改造。
//   u_g = golden Scheduler(全 golden 链: IssueQueue*/Entries*/...)
//   u_i = Scheduler_it(全重写链: xs_Scheduler_int_core + xs_IssueQueue*_core
//          + xs_Entries*_core + xs_iq_entry_* ; 仅 AgeDetector/FuBusyTable*/
//          MultiWakeupQueue/EnqPolicy/UIntCompressor 等结构原语沿用 golden)。
// 逐拍比对全部顶层功能输出(!$isunknown 跳 don't-care)。errors==0 即 PASS。
// 非功能口(重写侧主动简化的口)单列 nonfunc_errors, 打印不计入 PASS。
// =============================================================================
`timescale 1ns/1ps
module tb;
