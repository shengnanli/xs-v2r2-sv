#!/usr/bin/env python3
"""生成 TLBuffer_2 (TL A/D 两通道窄总线 skid 缓冲) 的 wrapper / 变体 / UT / Makefile / 黑盒。

TLBuffer_2 是叶子级结构模块: A/D 两通道各例化一个 Queue2 黑盒。
  · UT 双例化: golden TLBuffer_2 vs 可读 TLBuffer_2_xs, 两侧共用真实 Queue2/ram 叶子,
    随机驱动全部输入、逐拍比对全部输出。
  · FM: ref=golden TLBuffer_2 (Queue 自动黑盒), impl=pkg+可读核+wrapper+黑盒声明。
"""
...
UT = ROOT / "verif" / "ut" / "TLBuffer_2"

TOP = "TLBuffer_2"
CORE = "xs_TLBuffer_2_core"
VARIANT = "TLBuffer_2_xs"
QUEUES = ["Queue2_TLBundleA_2", "Queue2_TLBundleD_3"]
RAMS = ["ram_2x127", "ram_2x90"]
...
def gen_wrapper(ports):
    return ("// 自动生成：scripts/gen_tlbuffer2.py —— 勿手改\n"
...
def gen_blackbox():
    out = ["// 自动生成：scripts/gen_tlbuffer2.py —— 勿手改",
...
def gen_tb(ports):
    ...
    L = [
        ...
        "// TLBuffer_2 双例化逐拍比对: golden TLBuffer_2 vs 可读 TLBuffer_2_xs。",
        "`timescale 1ns/1ps",
...
def gen_makefile():
    ...
    return f"""# 自动生成：scripts/gen_tlbuffer2.py —— 勿手改
MODULE = TLBuffer_2

RTL_DIR = ../../../rtl
GOLDEN_RTL = ../../../golden/chisel-rtl

RTL_SRCS = $(RTL_DIR)/memblock/tlbuffer2_pkg.sv \\
           $(RTL_DIR)/memblock/TLBuffer_2.sv

TB_SRCS = variants_xs.sv tb.sv

# UT 双例化: 两侧共用真实 Queue2 + ram 叶子。
GOLDEN_SRCS = $(GOLDEN_RTL)/TLBuffer_2.sv \\
              {golden_q} \\
              {golden_r}

WRAPPER_SRCS = $(RTL_DIR)/memblock/TLBuffer_2_wrapper.sv \\
               $(RTL_DIR)/memblock/tlbuffer2_queue_blackbox.sv
FM_VARIANTS = TLBuffer_2
# FM: ref = golden TLBuffer_2.sv (Queue 自动黑盒); impl = pkg + 可读核 + wrapper + 黑盒声明。
...
def main():
    ...
    (RTL / "tlbuffer2_queue_blackbox.sv").write_text(gen_blackbox())
