...
// =============================================================================
  ...
  logic stageCheck1_fire_1cycle;
  always_ff @(posedge clock or posedge reset) begin
    if (reset) stageCheck1_fire_1cycle <= 1'b0;
    else       stageCheck1_fire_1cycle <= stageCheck1_fire;   // OneCycleValid(_, false)
  end
  // ValidHoldBypass(set, clr)：held' = ~clr & (set | held)（clr 优先于 set，与 golden 同）；
  //   输出再 OR 上当拍组合 set（rbv_resp），使 clr&set 同拍输出仍为 1。
  wire clr_resp = stageCheck1_fire_1cycle | refill_valid;
  ...
  for (gi = 0; gi < 4; gi++) begin : g_bypass_hold
    always_ff @(posedge clock or posedge reset) begin
      ...
      else       bypassed_hold[gi] <= ~clr_resp & (rbv_resp[gi] | bypassed_hold[gi]);
    end
    always_ff @(posedge clock or posedge reset) begin
  ...
  always_comb begin
    ...
    for (int s = 0; s < CONTIGUOUS; s++) begin
      ...
      resp_bits.stage1_entry[s].v =
        (resp_res.l0.hit ? resp_res.l0.v[s] :
        (resp_res.sp.hit ? resp_res.sp.v :
        (resp_res.l1.hit ? resp_res.l1.v :
        (resp_res.l2.hit ? resp_res.l2.v : resp_res.l3.v))));
