// =============================================================================
// xs_iq_entry_ffcfmacfdiv —— 香山 V2R2(昆明湖) 发射队列「单条目」可读重写
//                            (FaluFcvtF2vFmacFdiv 变体)
// -----------------------------------------------------------------------------
// 设计源:src/main/scala/xiangshan/backend/issue/{EntryBundles,EnqEntry,OthersEntry}.scala
// golden 对照:EnqEntry_8.sv / OthersEntry_88.sv(simp) / OthersEntry_90.sv(comp)
//
// ★ 乱序调度「唤醒-选择」的最小单元(浮点 FaluFcvtF2vFmacFdiv 变体)★
// 本文件是 FaluFmacFdiv 单条目核(IqEntryFfmacFdiv)的「近邻」fork:
// 唤醒匹配 / exuSources 压缩 / dataSources / canIssue / enqDelay / deqPortIdx /
//   issued / issueTimer 全部与 FaluFmacFdiv **逐字相同**,真重写仅两处:
//
//   ① fuType 多保留两位(FU_F2V=4 / FU_FCVT=13):enq/entry_update/输出
//      在 {11,12,14} 基础上再加 {4,13}(IQFuType.readFuType,共 5 位透传)。
//   ② payload 多两位写使能(vec_wen / v0_wen):因为是 packed struct,随 payload
//      整体寄存/透传,**无需额外逻辑**(唤醒匹配不看它们)。
//
// 其余机制(deqPortIdx 写/读、按端口选 issueResp、X 处理细则)同 FaluFmacFdiv,
// 详见 IqEntryFfmacFdiv.sv 头部,此处不赘述。
// =============================================================================
module xs_iq_entry_ffcfmacfdiv import iq_ffcfmacfdiv_pkg::*; #(
  ...
  // ===========================================================================
  // 2. 冲刷判定 flushed(环形指针比较,与 FaluFmacFdiv 同)
  // ===========================================================================
  logic flushed;
  always_comb begin
  ...
  //   注:vec_wen/v0_wen 不参与匹配——唤醒只看 fpWen(纯浮点 IQ)。
  // ===========================================================================
  ...
  // ===========================================================================
  // 4. 唤醒匹配组合逻辑(逐源)——与 FaluFmacFdiv 完全一致
  // ===========================================================================
  ...
  // ===========================================================================
  // 5. exuSources 编码:压缩器拓扑与 FaluFmacFdiv 同(bit{8,10,12},作 golden 黑盒)
  // ===========================================================================
  ...
  always_comb begin
    ...
    // payload 保持(含新增 vec_wen / v0_wen,随 struct 整体透传)
  ...
  // ===========================================================================
  // 9. 入队延迟唤醒(仅 IS_ENQ):与 FaluFmacFdiv 完全一致
  // ===========================================================================
  generate
