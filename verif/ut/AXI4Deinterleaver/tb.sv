...
module tb;
  ...
  logic [6:0] auto_out_b_bits_id;
  logic auto_out_ar_ready;
  logic auto_out_r_valid;
  logic [6:0] auto_out_r_bits_id;
  ...
  wire [6:0] g_auto_out_aw_bits_id;
  wire [6:0] i_auto_out_aw_bits_id;
  ...
  wire [6:0] g_auto_out_ar_bits_id;
  wire [6:0] i_auto_out_ar_bits_id;
  ...
  function automatic logic [6:0] rand_rid();
    case ($urandom_range(0, 3))
      0, 1: return 7'($urandom_range(0, 7));   // 小范围: 频繁碰撞/完成 burst
      2:    return 7'($urandom_range(0, 95));  // 全在用范围: 覆盖译码
      default: return 7'($urandom);            // 含越界 [96,127]: 覆盖 padding
    endcase
  endfunction
  ...
  initial begin
    ...
    $display("AXI4Deinterleaver checks=%0d errors=%0d", checks, errors);
