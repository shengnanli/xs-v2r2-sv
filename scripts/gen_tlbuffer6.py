#!/usr/bin/env python3
"""生成 TLBuffer_6 (TL A/D 两通道窄总线 skid 缓冲) 的 wrapper / 变体 / UT / Makefile / 黑盒。

TLBuffer_6 是叶子级结构模块: A/D 两通道各例化一个 Queue2 黑盒。
  · UT 双例化: golden TLBuffer_6 vs 可读 TLBuffer_6_xs, 两侧共用真实 Queue2/ram 叶子,
    随机驱动全部输入、逐拍比对全部输出。
  · FM: ref=golden TLBuffer_6 (Queue 自动黑盒), impl=pkg+可读核+wrapper+黑盒声明。
"""
...
UT = ROOT / "verif" / "ut" / "TLBuffer_6"

TOP = "TLBuffer_6"
CORE = "xs_TLBuffer_6_core"
VARIANT = "TLBuffer_6_xs"
QUEUES = ["Queue2_TLBundleA_6", "Queue2_TLBundleD_7"]
RAMS = ["ram_2x355", "ram_2x280"]
...
def gen_wrapper(ports):
    return ("// 自动生成：scripts/gen_tlbuffer6.py —— 勿手改\n"
...
def gen_blackbox():
    out = ["// 自动生成：scripts/gen_tlbuffer6.py —— 勿手改",
...
def gen_tb(ports):
    ...
    L = [
        ...
        "// TLBuffer_6 双例化逐拍比对: golden TLBuffer_6 vs 可读 TLBuffer_6_xs。",
        "`timescale 1ns/1ps",
...
def gen_makefile():
    ...
    return f"""# 自动生成：scripts/gen_tlbuffer6.py —— 勿手改
MODULE = TLBuffer_6

RTL_DIR = ../../../rtl
GOLDEN_RTL = ../../../golden/chisel-rtl

RTL_SRCS = $(RTL_DIR)/memblock/tlbuffer6_pkg.sv \\
           $(RTL_DIR)/memblock/TLBuffer_6.sv

TB_SRCS = variants_xs.sv tb.sv

# UT 双例化: 两侧共用真实 Queue2 + ram 叶子。
GOLDEN_SRCS = $(GOLDEN_RTL)/TLBuffer_6.sv \\
              {golden_q} \\
              {golden_r}

WRAPPER_SRCS = $(RTL_DIR)/memblock/TLBuffer_6_wrapper.sv \\
               $(RTL_DIR)/memblock/tlbuffer6_queue_blackbox.sv
FM_VARIANTS = TLBuffer_6
# FM: ref = golden TLBuffer_6.sv (Queue 自动黑盒); impl = pkg + 可读核 + wrapper + 黑盒声明。
...
def main():
    ...
    (RTL / "tlbuffer6_queue_blackbox.sv").write_text(gen_blackbox())
