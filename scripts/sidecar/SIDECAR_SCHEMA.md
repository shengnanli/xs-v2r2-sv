# FM sidecar schema + 证明契约(2026-07-19, v4 四审版)
...
| `objects` | `{blackbox_ref, blackbox_impl, interface_only, unmatched_ref, unmatched_impl}`,各为**排序·唯一·raw-path 列表**(非空、无控制符;见对象映射节) |
...
## 对象映射(四审: 显式声明, 不靠 emitter 猜名)

emitter **不做**模块名归一(多实例会折叠)。sidecar `objects` 保留 **raw path**
(如 `r:/WORK/Backend/inner_sub` / `i:/WORK/Backend/u_core/sub`)。
expectation 的 `allow` 各类别为**显式映射列表** `[{id, ref_path, impl_path}]`:
id 为 token(排序唯一), ref_path/impl_path 为 raw path。validator 校验
sidecar 的 ref/impl raw 路径集与映射的 ref_path/impl_path 集**精确相等**(按类别, 不可互换)。
