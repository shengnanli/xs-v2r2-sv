// 双端口 SRAM 变体包装层（FM impl 用，golden 同名）：
//   例化 xs_SRAMTemplate_2p_core + 厂商 2p SRAM 宏黑盒；端口与 Chisel golden 完全一致（flat），
//   仅作「数组打包 ↔ flat 端口」的机械适配。覆盖 SRAMTemplate_66(bp_sc 256set×4way×6bit)。
//   _xs 同体副本见 verif/ut/SRAMTemplate_2p/variants_xs.sv（UT 用）。
...
// ---- SRAMTemplate_64 (bp_tage_bt 256set×8way×2bit) ----
module SRAMTemplate_64(
  ...
  input         io_broadcast_ram_hold, io_broadcast_ram_bypass, io_broadcast_ram_bp_clken,
  input         io_broadcast_ram_aux_clk, io_broadcast_ram_aux_ckbp, io_broadcast_ram_mcp_hold,
  input  [63:0] io_broadcast_ram_ctl,
  input         io_broadcast_cgen,
  ...
  wire [7:0]  w_mask;
  xs_SRAMTemplate_2p_core #(.SET(256), .WAY(8), .DATA_WIDTH(2), .BORE_AW(9), .ENABLE_RESET(0)) u_core (
    .clock(clock), .reset(reset), .io_r_req_ready(),
    .io_r_req_valid(io_r_req_valid), .io_r_req_bits_setIdx(io_r_req_bits_setIdx),
    .io_r_resp_data({io_r_resp_data_7,io_r_resp_data_6,io_r_resp_data_5,io_r_resp_data_4,
                     io_r_resp_data_3,io_r_resp_data_2,io_r_resp_data_1,io_r_resp_data_0}),
    .io_w_req_valid(io_w_req_valid), .io_w_req_bits_setIdx(io_w_req_bits_setIdx),
    .io_w_req_bits_data({io_w_req_bits_data_7,io_w_req_bits_data_6,io_w_req_bits_data_5,io_w_req_bits_data_4,
                         io_w_req_bits_data_3,io_w_req_bits_data_2,io_w_req_bits_data_1,io_w_req_bits_data_0}),
    .io_w_req_bits_waymask(io_w_req_bits_waymask),
    .io_broadcast_ram_hold(io_broadcast_ram_hold), .io_broadcast_ram_bypass(io_broadcast_ram_bypass),
    .io_broadcast_ram_bp_clken(io_broadcast_ram_bp_clken), .io_broadcast_cgen(io_broadcast_cgen),
