...
## DELTA 轮次(parser 修复后重跑 124 盲区, @092f9f3, 2026-07-20)

**独立增量轮次**: 旧 fdb8e6b 基线冻结不覆盖; delta 证据独立根 signoff-evidence-delta/;
delta_ledger.tsv 标 round=DELTA/impl=092f9f3。124 之前全是 parser-blocked INFRA 盲区。

delta 124 新分类:
- **SIGNOFF_PASS 58**(盲区→真绿!含 AXI4IdIndexer/Arbiter1/DataModule/SyncDataModule/
  WrBypass/SRAMTemplate/IssueQueue/StoreQueue/LoadQueue 等——多是之前逐个 FM-passing 的模块)
- **PROOF_GAP 65**(全 PARTIAL; 有黑盒/unread, 须 assembly+policy 或 RTL 收敛)
- **INFRA_ERROR 1**(StorePipe: FM-081 golden 参考整体黑盒, golden 未读全, 真 setup 问题非 parser)

## 叠加后真实覆盖(基线 fdb8e6b + delta 092f9f3, 守恒 305)

| 类别 | 基线纠正后 | +delta | **叠加后** |
|---|---|---|---|
| **SIGNOFF_PASS** | 53 | +58 | **111**(36%) |
| SHADOW_CHECK | 1 | | 1 |
| **PROOF_GAP** | 103 | +65 | **168** |
| **INFRA_ERROR** | 148 | -123 | **25** |
| 合计 | 305 | | **305 ✓** |

**真绿覆盖率 53→111(17%→36%, 翻倍多)**——parser 盲区消除后, 一半盲区本就是可替换等价。
剩 168 PROOF_GAP 是真正待诊断/修 RTL 的清单; 25 INFRA(含 StorePipe golden-setup、
9 Tcl/setup、5 TIMEOUT 等)是 harness 待修。
