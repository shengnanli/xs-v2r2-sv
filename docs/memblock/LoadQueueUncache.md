...
`make fm` 进入 compare 后仍报告 FAILED/INCONCLUSIVE。按模块 Makefile 本地设置，FM 中
`UncacheEntry*`、`FreeList_6`、`RRArbiterInit_9`、`PipelineRegModule*` 均作为黑盒。

残留 failing 点共 20 个，当前全部落在 rollback 输出寄存器：
...
因此当前 FM 残留按项目方法判为黑盒/X/结构配对导致的 false positive，不为迁就 FM 改写成
firtool 风格命名。
