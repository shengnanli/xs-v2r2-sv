#!/usr/bin/env python3
"""
IssueQueueAluBrhJmpI2fVsetriwiVsetriwvfI2v / EntriesAluBrhJmpI2fVsetriwiVsetriwvfI2v 生成器(fork 自 gen_iq_abjivvi.py)。

可读核手写在 rtl/backend/{EntriesAluBrhJmpI2fVsetriwiVsetriwvfI2v,IssueQueueAluBrhJmpI2fVsetriwiVsetriwvfI2v}.sv,
通过 iq_abjivvi_pkg 的 struct/enum 表达条目阵列、转移策略、年龄选择。本脚本生成
「flat golden 端口 ↔ 可读核 struct 端口」的 wrapper 与双例化 tb。

相对 ACFD 样板的字段差异(本脚本据此调整 field map):
  * fuType 保留位 {0,1,2,3,6,28,29}(jmp/brh/i2f/i2v/alu/vsetiwi/vsetiwf);
  * payload 携带 preDecodeInfo.isRVC / pred_taken + fp/vec 写使能 + fpu 控制,无 flushPipe;
...
BK = XSSV / "rtl/backend"
UT = XSSV / "verif/ut/IssueQueueAluBrhJmpI2fVsetriwiVsetriwvfI2v"
GEN_HDR = "// 自动生成:scripts/gen_iq_abjivvi.py —— 勿手改\n"
...
def entry_field_expr(v, suffix):
    ...
    if m:
        ...
        fmap = {
            ...
            "fpWen": ".payload.fp_wen", "vecWen": ".payload.vec_wen",
            "v0Wen": ".payload.v0_wen", "vlWen": ".payload.vl_wen",
            "fpu_typeTagOut": ".payload.fpu_type_tag_out", "fpu_wflags": ".payload.fpu_wflags",
            "fpu_typ": ".payload.fpu_typ", "fpu_rm": ".payload.fpu_rm",
...
def emit_entries_wrapper():
    ports = parse_ports("EntriesAluBrhJmpI2fVsetriwiVsetriwvfI2v", "EntriesAluBrhJmpI2fVsetriwiVsetriwvfI2v.sv")
    pack, unpack = gen_entries_wrapper(ports)
    out = [GEN_HDR]
    out.append("// EntriesAluBrhJmpI2fVsetriwiVsetriwvfI2v golden 同名扁平 wrapper —— 例化可读核 xs_EntriesAluBrhJmpI2fVsetriwiVsetriwvfI2v_core。\n")
    out.append("module EntriesAluBrhJmpI2fVsetriwiVsetriwvfI2v import iq_abjivvi_pkg::*; (\n")
    ...
    out.append("  end\n")
    out.extend(u + "\n" for u in unpack)
    out.append("  xs_EntriesAluBrhJmpI2fVsetriwiVsetriwvfI2v_core u_core (\n")
    out.append(ENT_CORE_PORTS)
    out.append("  );\nendmodule\n")
    (BK / "EntriesAluBrhJmpI2fVsetriwiVsetriwvfI2v_wrapper.sv").write_text("".join(out))
    print("wrote EntriesAluBrhJmpI2fVsetriwiVsetriwvfI2v_wrapper.sv  (ports=%d pack=%d unpack=%d)"
...
def emit_entries_tb():
    ...
    L = []
    L.append("// 自动生成:scripts/gen_iq_abjivvi.py(entries tb)—— 勿手改\n")
    ...
    def inst(mod, pfx):
        ...
        s.append("\n  );\n")
        return "".join(s)
    L.append(inst("EntriesAluBrhJmpI2fVsetriwiVsetriwvfI2v", "g"))
    L.append(inst("EntriesAluBrhJmpI2fVsetriwiVsetriwvfI2v_xs", "i"))
...
def emit_entries_xs_variant():
    txt = (BK / "EntriesAluBrhJmpI2fVsetriwiVsetriwvfI2v_wrapper.sv").read_text()
    txt = txt.replace("module EntriesAluBrhJmpI2fVsetriwiVsetriwvfI2v ", "module EntriesAluBrhJmpI2fVsetriwiVsetriwvfI2v_xs ")
    (UT / "entries_variant_xs.sv").write_text(txt)
    print("wrote entries_variant_xs.sv")


def emit_iq_svh():
    ports = parse_ports("IssueQueueAluBrhJmpI2fVsetriwiVsetriwvfI2v", "IssueQueueAluBrhJmpI2fVsetriwiVsetriwvfI2v.sv")
    p = [GEN_HDR.replace("\n", " (顶层端口) \n")]
    for d, w, n in ports:
        p.append(decl(d, w, n))
    p[-1] = p[-1].rstrip(",\n") + "\n"
    (BK / "issuequeue_abjivvi_ports.svh").write_text("".join(p))

    c = [GEN_HDR.replace("\n", " (顶层连线) \n")]
    c.append("  // 顶层端口与可读核完全同名, 纯穿透例化。\n")
    c.append("  xs_IssueQueueAluBrhJmpI2fVsetriwiVsetriwvfI2v_core u_core (\n")
    c.append("    .clock(clock), .reset(reset),\n")
    conn = [f"    .{n}({n})" for d, w, n in ports]
    c.append(",\n".join(conn))
    c.append("\n  );\n")
    (BK / "issuequeue_abjivvi_connect.svh").write_text("".join(c))
    print("wrote issuequeue_abjivvi_{ports,connect}.svh (ports=%d)" % len(ports))
...
def emit_iq_tb():
    ...
    outs = [(w, n) for d, w, n in ports if d == "output"]
    L = []
    L.append("// 自动生成: scripts/gen_iq_abjivvi.py(iq tb)—— 勿手改\n")
    L.append("`timescale 1ns/1ps\nmodule tb;\n")
    L.append("  int unsigned NCYCLES = 200000;\n")
    L.append("  bit clk=0, rst; int errors=0, checks=0;\n")
    ...
    def inst(mod, pfx):
        ...
        s.append("\n  );\n")
        return "".join(s)
    L.append(inst("IssueQueueAluBrhJmpI2fVsetriwiVsetriwvfI2v", "g"))
    L.append(inst("IssueQueueAluBrhJmpI2fVsetriwiVsetriwvfI2v_xs", "i"))
...
def emit_iq_xs_variant():
    L = [GEN_HDR.replace("\n", " (顶层 _xs 变体) \n")]
    L.append("module IssueQueueAluBrhJmpI2fVsetriwiVsetriwvfI2v_xs (\n")
    L.append("  input  clock,\n  input  reset,\n")
    L.append('`include "issuequeue_abjivvi_ports.svh"\n')
    L.append(");\n")
    L.append('`include "issuequeue_abjivvi_connect.svh"\n')
    L.append("endmodule\n")
