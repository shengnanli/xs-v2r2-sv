// =============================================================================
// xs_iq_entry_abjivvi —— 香山 V2R2(昆明湖) 发射队列「单条目」可读重写
//                        (AluBrhJmpI2fVsetriwiVsetriwvfI2v 变体, IQ 变体收官)
// -----------------------------------------------------------------------------
// 设计源:src/main/scala/xiangshan/backend/issue/{EntryBundles,EnqEntry,OthersEntry}.scala
// golden 对照:EnqEntry_4.sv(入队条目) / OthersEntry_44.sv(简单条目) / OthersEntry_50.sv(复杂条目)
//
// ★ 这是乱序调度「唤醒-选择」机制的最小单元 ★(机理同 AluMulBkuBrhJmp 样板,见 IqEntryAmbb.sv)
// 本变体相对样板的**唯一实质差异**集中在两处,其余唤醒/取消/RegCache/loadDep/issued/
// issueTimer/transEntry/resp 逻辑与样板逐行同构(同一颗 UIntCompressor、同一组 og0Cancel
// 命中位 {0,2,4,6}、同样 is0Lat 仅源 0/1、同样 issueTimer-only 的 resp):
//   A) FuType 保留位:本变体 = {jmp(0),brh(1),i2f(2),i2v(3),alu(6),vsetiwi(28),vsetiwf(29)};
//      AMBB 是 {jmp,brh,alu,mul,bku}。仅影响 entry_update.status.fu_type / o_fu_type 按位保留。
//   B) payload 字段:本变体在分支预测 isRVC/pred_taken 之外,还携带 fp/vec 写使能
//      fpWen/vecWen/v0Wen/vlWen 与浮点控制 fpu.{typeTagOut,wflags,typ,rm}。
//      ——这些字段都是纯透传(随条目寄存/转移搬运),不参与唤醒选择逻辑,故核内不需特殊处理
//        (entry_update.payload <= entry_reg.payload 已整体搬运)。
//
// 三种条目变体由参数选择(对应 golden 三个 leaf):
//   IS_ENQ=1            入队条目(EnqEntry_4):带 2 拍「入队延迟唤醒」流水。
//   IS_ENQ=0,IS_COMP=0 简单条目(OthersEntry_44):无 canIssueBypass。
//   IS_ENQ=0,IS_COMP=1 复杂条目(OthersEntry_50):有 canIssueBypass + 即时前递输出。
//
// X 处理与样板一致:状态全在 entry_reg(复位无关,UT 用 `!$isunknown` + initreg 收敛);
// 三元 mux / if-else 选择避免 X 传播;exuSources 编码作 golden 黑盒 UIntCompressor。
// =============================================================================
module xs_iq_entry_abjivvi import iq_abjivvi_pkg::*; #(
  ...
  //   注:本变体 golden EnqEntry_4 仅有 enqDelayIn1(单级延迟唤醒),无 enqDelayIn2。
  //   整数路 dataSources 只取 bypass,delay1 已覆盖 srcState/loadDep/regcache,故不实现
  //   delay2,亦不引 delay2 输入(移除后消除 Entries 顶层死流水寄存器,与 AMBB 一致)。
  input  wk_wb_t [NUM_WK_WB-1:0]       wk_wb_d1,
  input  wk_iq_t [NUM_WK_IQ-1:0]       wk_iq_d1,
  // RegEnable 后的入队源 load 依赖,只含被读的 bit[1](loadCancel 只看 dep[p][1])
  input  logic [NUM_REGSRC-1:0][LDPW-1:0] enqd1_src_ld,
  ...
  // 同上,但 ld2Cancel 已窄化为 [LDPW-1:0] 位向量(delay1 路只寄存 ld2Cancel,见端口注释)。
  ...
  // enqDelayIn1 srcLoadDep 已窄化为只含被读的 dep[p][1] 位向量([LDPW-1:0]),直接与 ld2Cancel 与。
  function automatic logic load_should_cancel_ld2_bit1(input logic [LDPW-1:0] dep1,
                                                       input logic [LDPW-1:0] ld2c);
    logic c;
    c = 1'b0;
    for (int p = 0; p < LDPW; p++) c |= ld2c[p] & dep1[p];
    return c;
  endfunction
  ...
  // og0Cancel 对各唤醒源的取消位(拓扑与 AMBB 一致:仅源 0/1 有 is0Lat,源 0..3 被 og0 取消)
  always_comb begin
    iq_cancel_sel = '0;
  ...
  // ===========================================================================
  // 6. exuSources 编码(同 AMBB:UIntCompressor 黑盒,输入布局 bit{0,2,4,6}+{20,21,22})
  // ===========================================================================
  ...
  // ===========================================================================
  // 8. shiftedWakeupLoadDependency(同 AMBB)
  // ===========================================================================
  ...
  always_comb begin
    ...
    // fuType 已按 FU_TYPE_KEEP_MASK 紧凑存储(仅本变体相关 7 位),整体透传即可
    ...
    entry_update.payload = entry_reg.payload; // payload(含 fp/vec/fpu/分支预测)整体透传
  end
  ...
  if (IS_ENQ) begin : g_enq_delay
    ...
    always_comb begin
      ...
      for (int s = 0; s < NUM_REGSRC; s++) begin
        ...
        d1_ldcancel[s] = load_should_cancel_ld2_bit1(enqd1_src_ld[s], enqd1_ldcancel);
        for (int p = 0; p < LDPW; p++) begin
  ...
  // fuType 输出(IQFuType.readFuType:紧凑保留位复原成 35 位 one-hot)
