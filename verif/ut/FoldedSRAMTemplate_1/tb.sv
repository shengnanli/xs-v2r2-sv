// FoldedSRAMTemplate_1 UT：golden vs _xs（均例化 golden 内层 SplittedSRAMTemplate_4）。
// width=1（不折叠），纯透传 TAGE 表 {valid,tag,ctr}。
`timescale 1ns/1ps
module tb;
  ...
  FoldedSRAMTemplate_1    u_g (`PORTS(g_ready,g_v0,g_t0,g_c0,g_v1,g_t1,g_c1,g_brd));
  FoldedSRAMTemplate_1_xs u_i (`PORTS(i_ready,i_v0,i_t0,i_c0,i_v1,i_t1,i_c1,i_brd));
