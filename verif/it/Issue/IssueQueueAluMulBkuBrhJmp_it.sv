// =============================================================================
// xs_IssueQueueAluMulBkuBrhJmp_core —— 香山 V2R2 发射队列 AluMulBkuBrhJmp 变体 可读核
// -----------------------------------------------------------------------------
// 设计源: src/main/scala/xiangshan/backend/issue/IssueQueue.scala (class IssueQueue)
//
// 本核与样板 AluCsrFenceDiv 同骨架(enq 分配 / canIssue / 三段年龄仲裁 / Mux1H 出队 /
// 一拍 deqDelay / 唤醒队列 / validCnt 统计),增量集中在以下几处(从 Scala 意图真重写,
// 不套壳):
//
//   1) 功能单元集合 / 端口分工:
//        fuType 保留 {brh(0),jmp(1),alu(6),mul(7),bku(10)};
//        端口0 = {alu,mul,bku}=fuType{6,7,10}; 端口1 = {brh,jmp}=fuType{0,1}。
//   2) 忙表(busy table)双重筛选(本变体新增 WbBusyTable):
//        端口0 = canIssue & ~fuBusyTable(Mul 多周期独占)& ~intWbBusyTable[端口0];
//        端口1 = canIssue & ~intWbBusyTable[端口1]。
//      fuBusyTable / intWbBusyTable 的读写表(FuBusyTableRead/Write)作 golden 黑盒例化,
//      其写侧吃「本拍发射(deqBeforeDly_0)+og0Resp」的 fuType,读侧吐各条目 busy mask。
//      intWbBusyTable 的读输入/写输出还引到 IQ 顶层 io 端口(与上层写回总线对接)。
//   3) 0 周期唤醒(is0Lat):唤醒广播 io_wakeupToIQ_0.bits.is0Lat =
//        ~(deqFuType[7] | deqFuType[10]),即只有非 Mul/Bku 才是 0 延迟旁路。
//        唤醒队列 enq 还带 lat = {Mul|Bku, 0}(多周期源延后弹出)。
//   4) payload:携带分支预测 preDecodeInfo.isRVC / pred_taken / ftq(送端口1 BrhJmp),
//        deqDelay 两端口都带 immType;端口1 额外带 preDecode/predictInfo/ftq。无 flushPipe。
//   5) validCntDeqVec:两个端口各自统计「在队且本端口可接(deqCanAccept)」的 uop 数。
//
// 子模块(entries / FuBusyTableRead{,_2} / FuBusyTableWrite / MultiWakeupQueue /
//  NewAgeDetector / AgeDetector{,_1})均作 golden 黑盒例化, 由本核连线。
// =============================================================================
module xs_IssueQueueAluMulBkuBrhJmp_core import iq_ambb_pkg::*; (
  input         clock,
  input         reset,
  ...
  // 两端口共用一个 struct;端口1(BrhJmp)专用字段(preDecode/predict/ftq)端口0 不用。
  // ===========================================================================
  typedef struct packed {
    ...
    logic [34:0] fu_type;       // 功能单元 one-hot(只保留 0/1/6/7/10 位)
    ...
    logic [7:0]  pdest;
    logic        rf_wen;
    logic        pre_decode_isrvc; // 仅端口1
    logic        ftq_flag;         // 仅端口1
    logic [5:0]  ftq_value;        // 仅端口1
    logic [3:0]  ftq_offset;       // 仅端口1
    logic        predict_taken;    // 仅端口1
    logic [3:0]  ds0;
    logic [3:0]  ds1;
    logic [2:0]  exu0;
    logic [2:0]  exu1;
  ...
  // deqEntry 端口
  wire        e_deq0_robflag, e_deq0_ft0, e_deq0_ft1, e_deq0_ft6, e_deq0_ft7, e_deq0_ft10;
  ...
  wire        e_deq0_rfwen;
  wire        e_deq1_robflag, e_deq1_ft0, e_deq1_ft1, e_deq1_ft6, e_deq1_ft7, e_deq1_ft10;
  ...
  // ---- 忙表输出 ----
  wire [23:0] fu_busy_mask;       // 端口0 功能单元忙表(Mul 独占)
  wire [23:0] intwb_busy_mask_0;  // 端口0 整数写回忙表
  wire [23:0] intwb_busy_mask_1;  // 端口1 整数写回忙表
  wire [2:0]  fu_busy_table;      // FuBusyTableWrite 内部表(fuBusyTableWrite_0 → fuBusyTableRead_0)
  // ---- wakeup 队列输出 ----
  wire        wkq_deq_valid, wkq_deq_rfwen;
  wire [34:0] wkq_deq_futype;
  ...
  // ---------------------------------------------------------------------------
  //   端口0 = canIssue & ~fuBusyTable & ~intWbBusyTable[0], 再 & {alu,mul,bku}(fuType 6/7/10);
  //   端口1 = canIssue & ~intWbBusyTable[1],               再 & {brh,jmp}(fuType 0/1)。
  // ===========================================================================
  wire [23:0] canIssueVec = e_can_issue;
  wire [23:0] canIssueMergeAllBusy_0 = canIssueVec & ~fu_busy_mask & ~intwb_busy_mask_0;
  wire [23:0] canIssueMergeAllBusy_1 = canIssueVec & ~intwb_busy_mask_1;
  ...
  generate
    for (gi = 0; gi < 24; gi++) begin : g_portfu
      assign port0_fu[gi] = e_fu_type[gi][FU_ALU] | e_fu_type[gi][FU_MUL] | e_fu_type[gi][FU_BKU];
  ...
  // 端口fuType 重组(deqEntry 只给 0/1/6/7/10 位, 拼回 35 位 one-hot)。
  wire [34:0] deqBeforeDly_0_fuType =
    {24'h0, e_deq0_ft10, 2'h0, e_deq0_ft7, e_deq0_ft6, 4'h0, e_deq0_ft1, e_deq0_ft0};
  wire [34:0] deqDly0_fuType =
    {24'h0, e_deq0_ft10, 2'h0, e_deq0_ft7, e_deq0_ft6, 4'h0, e_deq0_ft1, e_deq0_ft0};
  wire [34:0] deqDly1_fuType =
    {24'h0, e_deq1_ft10, 2'h0, e_deq1_ft7, e_deq1_ft6, 4'h0, e_deq1_ft1, e_deq1_ft0};
  // 送忙表/唤醒队列的 deqResp fuType(端口0)
  wire [34:0] toBusyTableDeqResp_0_fuType = deqDly0_fuType;
  ...
  // deqDelay 输出绑定(端口1 = BrhJmp,带分支预测字段)
  ...
  // ===========================================================================
  // 7. wakeupToIQ: 端口0 发射结果进 MultiWakeupQueue, 形成唤醒广播。
  //   is0Lat = ~(deqFuType[7] | deqFuType[10])(非 Mul/Bku 才 0 延迟)。
  // ===========================================================================
  assign io_wakeupToIQ_0_valid       = wkq_deq_valid;
  assign io_wakeupToIQ_0_bits_rfWen  = wkq_deq_valid & wkq_deq_rfwen;
  assign io_wakeupToIQ_0_bits_is0Lat = ~(wkq_deq_futype[FU_MUL] | wkq_deq_futype[FU_BKU]);
  assign io_wakeupToIQ_0_bits_rcDest = io_replaceRCIdx_0;

  // ===========================================================================
  // 8. 子模块例化(均为 golden 黑盒)
  // ===========================================================================
  EntriesAluMulBkuBrhJmp_it entries (
    ...
    .io_deqEntry_0_bits_status_fuType_7 (e_deq0_ft7),
    .io_deqEntry_0_bits_status_fuType_10 (e_deq0_ft10),
    ...
    .io_deqEntry_1_bits_status_fuType_7 (e_deq1_ft7),
    .io_deqEntry_1_bits_status_fuType_10 (e_deq1_ft10),
  ...
  // ---- 端口0 功能单元忙表(Mul 多周期独占):写表吃发射+og0Resp 的 fuType ----
  FuBusyTableWrite fuBusyTableWrite_0 (
    ...
    .io_out_fuBusyTable        (fu_busy_table)
  );
  FuBusyTableRead fuBusyTableRead_0 (
    .io_in_fuBusyTable      (fu_busy_table),
  ...
  // ---- 整数写回忙表(端口0):写输出引到 IQ 顶层 io,读输入来自顶层 io ----
  FuBusyTableWrite intWbBusyTableWrite_0 (
    .clock (clock), .reset (reset),
    .io_in_deqResp_valid       (deqBeforeDly_0_valid & e_deq0_rfwen),
    .io_in_deqResp_bits_fuType (toBusyTableDeqResp_0_fuType),
    .io_in_og0Resp_valid       (io_og0Resp_0_valid),
    .io_in_og0Resp_bits_fuType (io_og0Resp_0_bits_fuType),
  ...
  // ---- 整数写回忙表(端口1):FuBusyTableRead_2(读输入 1 位)----
  FuBusyTableRead_2 intWbBusyTableRead_1 (
    ...
    .io_in_fuTypeRegVec_20  (e_fu_type[20]), .io_in_fuTypeRegVec_21 (e_fu_type[21]),
    .io_in_fuTypeRegVec_22  (e_fu_type[22]), .io_in_fuTypeRegVec_23 (e_fu_type[23]),
    .io_out_fuBusyTableMask (intwb_busy_mask_1)
  );
  ...
  MultiWakeupQueue wakeUpQueues_0 (
    ...
    .io_enq_bits_uop_fuType             (toBusyTableDeqResp_0_fuType),
    .io_enq_bits_uop_robIdx_flag        (e_deq0_robflag),
    .io_enq_bits_uop_robIdx_value       (e_deq0_robval),
    ...
    .io_enq_bits_lat                    ({e_deq0_ft7 | e_deq0_ft10, 1'h0}),
    .io_deq_valid                       (wkq_deq_valid),
    .io_deq_bits_fuType                 (wkq_deq_futype),
...
// =============================================================================
// IssueQueueAluMulBkuBrhJmp_it —— golden 同名顶层。可读核接口已持平 golden 扁平接口,
// 故为纯穿透 wrapper。
// =============================================================================
module IssueQueueAluMulBkuBrhJmp_it (
  input         clock,
  input         reset,
`include "issuequeue_ambb_ports.svh"
);
`include "issuequeue_ambb_connect.svh"
endmodule
