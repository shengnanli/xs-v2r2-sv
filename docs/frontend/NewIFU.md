...
```mermaid
flowchart LR
    FTQ["FTQ 取指请求<br/>(ftqPtr / startAddr)"] --> F0
    F0["f0：算取指起止<br/>向 ICache 发请求"] --> F1
    F1["f1：算各槽 PC<br/>(pc_lower+高位/snpc)<br/>准备 cut"] --> F2
    F2["f2：收 ICache 数据<br/>cut 出 16 取指槽<br/>初步 PreDecode→f3_pd_wire"] --> F3
    F3["f3：F3Predecoder 重算 brType/isCall/isRet<br/>RVCExpander 展开<br/>PredChecker 比对预测"]
    IC["ICache 响应（数据/异常/way）"] --> F2
    F3 --> IBUF["送 IBuffer（指令+元数据）"]
    F3 -->|预测错| RDR["redirect → FTQ"]
    F2 -.MMIO 旁路.-> MMIO["MMIO FSM（见 §3.4）<br/>逐条串行取，每块 1 条"]
    MMIO -.提交点.-> F3
```

> 图注：正常路径 f0→f3 四级流水（f2 收数据/初译、f3 重算分支信息并校验）。MMIO（非缓存）
> 取指走旁路状态机，串行完成后在提交点汇回 f3 送 IBuffer/发 redirect。各级 16 取指槽以
> 数组+genvar 并行。
...
转移条件对照 RTL（`rtl/frontend/NewIFU.sv:769-816`）。后端 redirect 冲刷自身或被更早分支冲刷
（`f3_ftq_flush_self || f3_ftq_flush_by_older`）时一律复位到 `M_IDLE`（优先级最高，图中省略该全局边）。

```mermaid
stateDiagram-v2
    [*] --> M_IDLE
    M_IDLE --> M_SEND_REQ: f3_req_is_mmio &&<br/>pbmt==NC
    M_IDLE --> M_WAIT_LAST_CMT: f3_req_is_mmio &&<br/>pbmt!=NC
    M_WAIT_LAST_CMT --> M_SEND_REQ: is_first_instr ||<br/>mmioLastCommit
    M_SEND_REQ --> M_WAIT_RESP: uncache_req fire
    M_WAIT_RESP --> M_SEND_TLB: fromUncache_fire &&<br/>needResend
    M_WAIT_RESP --> M_WAIT_COMMIT: fromUncache_fire &&<br/>!needResend
    M_SEND_TLB --> M_TLB_RESP: itlb_req fire
    M_TLB_RESP --> M_WAIT_COMMIT: itlb_resp &&<br/>有 tlb 异常
    M_TLB_RESP --> M_SEND_PMP: itlb_resp &&<br/>无异常
    M_SEND_PMP --> M_WAIT_COMMIT: 有 pmp 异常
    M_SEND_PMP --> M_RESEND_REQ: 无异常
    M_RESEND_REQ --> M_WAIT_RESEND: uncache_req fire
    M_WAIT_RESEND --> M_WAIT_COMMIT: fromUncache_fire
    M_WAIT_COMMIT --> M_COMMITED: mmio_commit ||<br/>pbmt==NC
    M_COMMITED --> M_IDLE
    M_COMMITED --> [*]
```

> 图注：11 态串行链 InstrUncache→iTLB→PMP→重发→等 RoB 提交。跨字重取分支
> (`M_WAIT_RESP`→`M_SEND_TLB`)；iTLB/PMP 任一异常都短路到 `M_WAIT_COMMIT`。
