// =============================================================================
// DelayReg_xs —— UT 用可读核包装(与 rtl/backend/DelayReg_wrapper.sv 逻辑一致,
//   仅模块名不同,以便与 golden DelayReg 同时例化逐拍比对)。
// =============================================================================
module DelayReg_xs
  import delayreg_pkg::*;
(
  ...
  xs_delay_reg_core #(.WIDTH($bits(diff_commit_t)), .N(DELAY_N)) u_core (
    .clock(clock), .reset(reset), .in(din), .out(dout));
