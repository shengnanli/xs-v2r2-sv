// =============================================================================
// xs_EntriesVfdivVidiv_core —— 香山 V2R2(昆明湖) 发射队列条目阵列 可读重写
//                              (VfdivVidiv 变体, hasCompAndSimp 路)
// -----------------------------------------------------------------------------
// 设计源:src/main/scala/xiangshan/backend/issue/Entries.scala
// golden 对照:EntriesVfdivVidiv.sv(例化 2×EnqEntry_18 + 2×OthersEntry_164(simp)
//             + 6×OthersEntry_166(comp) + 2×EnqPolicy_18(simp/comp 转移策略))
//
// ★ 本模块 = 条目阵列 + 转移策略(transfer policy) + 年龄最老选择(oldest mux)★
// 单条目「唤醒-选择」逻辑全在 xs_iq_entry_vfdiv(IqEntryVfdiv.sv);本层做:
//   1) 例化 10 个单条目:2 enq(IS_ENQ) + 2 simp(IS_TRANS) + 6 comp。
//   2) 转移策略(hasCompAndSimp 分支):simp 为转移源,提级到 comp。
//   3) 年龄最老选择(numDeq=1):comp 命中优先于 simp 优先于 enq(三级 Mux)。
//   4) 阵列状态汇聚:valid/issued/canIssue/fuType/dataSources。
//
// ★ 与浮点样板(EntriesFaluFmac)的差异 ★
//   * numEntries 10(2/2/6)、numRegSrc 5、无 IQ 唤醒 → 无 exuSources/og0Cancel/
//     enqDelay 取消寄存器。enqDelay 仅把当拍 WB 总线打 1 拍喂给 EnqEntry。
//   * issueResp 单端口,本变体用 og2Resp(向量除法多周期,timer 走到 2 才出真响应):
//       valid[timer] = {timer0→og0.valid, timer1→og1.valid, timer2→og2.valid, timer3→0}
//       resp [timer] = {timer0→BLOCK, timer1→UNCERTAIN, timer2→og2.bits.resp, timer3→0}
//     (浮点变体是 timer1→SUCCESS;向量除法把真正的成功/阻塞推迟到 og2。)
//   * 多 vl_info(vlFromInt/Vf 的 zero/vlmax)透传给各条目做 ignoreOldVd。
//
// X 铁律:oldest / Mux1H 用「sel[i] ? entry[i] : 0」OR 累加,sel=0 不引 X。
// =============================================================================
module xs_EntriesVfdivVidiv_core import iq_vfdiv_pkg::*; (
  input  logic clock,
  input  logic reset,
  ...
  // ===========================================================================
  // 0. 条目分区:索引 [0..1]=enq, [2..3]=simp(2), [4..9]=comp(6)
  // ===========================================================================
  ...
  // ===========================================================================
  // 1. issueRespVec:resp/valid = f(issueTimer)。本变体用 og2Resp(timer2 出真响应)。
  // ===========================================================================
  always_comb begin
    for (int e = 0; e < NUM_ENTRIES; e++) begin
      ...
      if (tmr == 2'h0) begin
        ...
        v = og2resp_valid;       r = resp_type_e'(og2resp_resp);
      end
  ...
  // EnqPolicy 黑盒输入 canEnq(NUM_OTHERS=8 位):
  ...
  EnqPolicy_18 u_simp_trans_policy (
  ...
  EnqPolicy_18 u_comp_trans_policy (
  ...
  // finalCompTransSelVec(enq):两支都用 selValid gate 的 selOH(与浮点样板同构)。
  ...
  // ===========================================================================
  // 5. 例化 10 个单条目。enq 延迟唤醒:把当拍 WB 总线打 1 拍(由顶层提供 wk_wb_d)。
  // ===========================================================================
  genvar e;
  generate
    for (e = 0; e < NUM_ENTRIES; e++) begin : g_entry
      ...
      xs_iq_entry_vfdiv #(.IS_ENQ(IS_ENQ_E), .IS_TRANS(IS_TRANS_E)) u_ety (
        .clock          (clock),
        .reset          (reset),
