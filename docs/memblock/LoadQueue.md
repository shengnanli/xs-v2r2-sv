...
> `Verification SUCCEEDED`，58691 passing / 0 failing / 0 unverified**。
...
- **FM**：冻结基线全貌重跑（`fm_full.log`）**原生 `Verification SUCCEEDED`：58691 passing /
  0 failing / 0 unverified**（两侧读入真实 golden 子队列定义）。历史注脚：冻结前的部分验证
  曾因 impl 侧子队列例化在重命名实例 + 额外 `u_core` 层次下与 ref 侧同名不同路径、比对点
  配对不收敛（大量子队列内部寄存器 unmatched 被当作自由变量产生级联失配），当时以 UT 充分
  为权威；冻结基线重跑后按名配对收敛（58605 by name + 86 by signature），FM 已原生通过。
